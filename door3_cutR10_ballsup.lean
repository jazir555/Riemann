...
/-! # APPEND-10 (FE honest attempt + banked rechain + triple gap file; append-only tail):

Grep record (read before writing, APPEND-9 tail :2244-2324):
* Gamma best half CLOSED `≤ 1 / 2`: `cutR10_gamma_sup_half_closed :1358`,
  rechains `:2143`, `:2200`, `:2273`.
* Zeta edges CLOSED: `cutR10_zeta_rightSliver_closed :1073` (`Re ≥ 3/2 → ≤ 3`),
  `cutR10_zeta_rightEdge_two_closed :1640` (`Re ≥ 2 → ≤ 2`),
  rechains `:2280`, `:2285`.
* Reflected Euler CLOSED `cutR10_reflected_Euler_two_closed :1920`
  (`t.re ≤ -1 → ‖zeta (1 - t)‖ ≤ 2`); bridge
  `cutR10_zeta_of_chi_reflected :1930`; sliver
  `cutR10_hFE_sliver_of_chi :1948` (needs chi `≤ 3`); full
  `cutR10_hFE_of_chi_and_middle :1962` (needs chi `≤ 3` + middle `≤ 2`).
* Chi `≤ 3`: no banked lemma locally (record `:1864-1869`); grep for
  chi norm finds only `:1933`, `:1951`, `:1965` premises, no closed cap.
* Gamma `1 / 100` OPEN on `hProdCap` (`:2297`); zeta `≤ 6` OPEN on `hFE`
  (`:2305` via `:1701`); joint `0.04` OPEN on `hJoint` (`:2314` via `:946`).
* Joint banked sliver CLOSED `1072 / 5` via
  `cutR10_sliver_sup_banked_closed :2064`, rechain `:2290`.

Verdict here:
* FE premise NOT closed: reflected half re-chained below by direct call;
  sliver and full hFE re-stated below as exact conditionals on the open
  chi cap (`≤ 3`) and open middle reflected cap (`≤ 2`); full-rect `≤ 6`
  therefore stays gated on single explicit `hFE`.
* Best banked Gamma stays `1 / 2`; best banked zeta stays `3` / `2` on edges;
  best banked joint stays `1072 / 5` on the `Re ≥ 2` sliver (rechained below
  by direct call, no new analysis).
* Full-rect `1 / 100` stays gated on single explicit `hProdCap`;
...
/-- Gamma best banked half rechain (`≤ 1 / 2`). -/
theorem cutR10_gamma_best_half_rechain10 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ)) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 2 :=
  cutR10_gamma_sup_half_closed s hlo hhi hilo hihi
...
/-- Zeta best banked edge rechain (`Re ≥ 3/2 → ≤ 3`). -/
theorem cutR10_zeta_edge_three_rechain10 (s : ℂ) (hs : 3 / 2 ≤ s.re) :
    ‖zeta s‖ ≤ 3 :=
  cutR10_zeta_rightSliver_closed s hs

/-- Zeta best banked edge rechain (`Re ≥ 2 → ≤ 2`). -/
theorem cutR10_zeta_edge_two_rechain10 (s : ℂ) (hs : (2 : ℝ) ≤ s.re) :
    ‖zeta s‖ ≤ 2 :=
  cutR10_zeta_rightEdge_two_closed s hs

/-- Reflected Euler half rechain CLOSED (`t.re ≤ -1 → reflected ≤ 2`). -/
theorem cutR10_reflected_Euler_two_rechain10 (t : ℂ)
    (ht : t.re ≤ (-1 : ℝ)) :
    ‖zeta (1 - t)‖ ≤ 2 :=
  cutR10_reflected_Euler_two_closed t ht

/-- Sliver hFE honest conditional: Euler sliver `≤ 3 * 2` given chi `≤ 3`. -/
theorem cutR10_hFE_sliver_conditional_rechain10 (t : ℂ)
    (hlo : (-1.06 : ℝ) ≤ t.re) (hhi : t.re ≤ (-1 : ℝ))
    (hilo : (8.44 : ℝ) ≤ t.im) (hihi : t.im ≤ (11.56 : ℝ))
    (hChi : ‖cutR10_chiFE (1 - t)‖ ≤ 3) :
    ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_sliver_of_chi t hlo hhi hilo hihi hChi

/-- Full hFE honest conditional: exact `hFE` shape given chi `≤ 3` plus
middle reflected `≤ 2`; both premises stay open locally. -/
theorem cutR10_hFE_full_conditional_rechain10
    (hChi : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) →
      ‖cutR10_chiFE (1 - t)‖ ≤ 3)
    (hMid : ∀ u : ℂ, (-0.50 : ℝ) ≤ u.re → u.re ≤ (2.06 : ℝ) →
    ...
    ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_of_chi_and_middle hChi hMid

/-- Joint best banked rechain on the in-ball `Re ≥ 2` sliver (`≤ 1072 / 5`). -/
theorem cutR10_joint_sliver_rechain10 (z : ℂ)
    (hz : z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1))
    (hs2 : (2 : ℝ) ≤ (shiftedS z).re) :
    ‖xiShiftedEntire z‖ ≤ (1072 / 5 : ℝ) :=
  cutR10_sliver_sup_banked_closed z hz hs2
...
/-- Gamma gap: `1 / 100` gated on single explicit `hProdCap`. -/
theorem cutR10_gamma_hundred_gap10 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hProdCap : ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100 :=
...
/-- Zeta gap: full-rect `≤ 6` gated on single explicit `hFE`. -/
theorem cutR10_zeta_six_gap10 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hFE : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2) :
...
/-- Joint gap: exact `0.04` gated on single explicit `hJoint`. -/
theorem cutR10_joint_exact_gap10
    (hJoint : ∀ z : ℂ, z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1) →
      ‖(1 / 2 : ℂ) * shiftedS z * (shiftedS z - 1)‖ *
        ‖((Real.pi : ℂ) ^ (-(shiftedS z / 2)))‖ *
        ‖Complex.Gamma (shiftedS z / 2)‖ * ‖zeta (shiftedS z)‖
...
/-! # APPEND-11 (closeout audit + banked rechain; append-only tail):

Grep record (read before writing, APPEND-10 tail :2326-2440):
* Gamma best half `≤ 1 / 2`: base `:1358`, rechain10 `:2362`.
* Zeta edges: `≤ 3` on `Re ≥ 3 / 2` base `:1073`, rechain10 `:2369`;
  `≤ 2` on `Re ≥ 2` base `:1640`, rechain10 `:2374`.
* Reflected Euler `≤ 2` on `t.re ≤ -1`: base `:1920`, rechain10 `:2379`.
* FE sliver conditional base `:1948`, rechain10 `:2385`
  (needs `‖cutR10_chiFE (1 - t)‖ ≤ 3`); FE full conditional base `:1962`,
  rechain10 `:2394` (needs chi `≤ 3` + middle reflected `≤ 2`).
* Joint sliver `≤ 1072 / 5` on in-ball `Re ≥ 2`: base `:2064`,
  rechain10 `:2406`.
* Gaps: Gamma `1 / 100` on `hProdCap` `:2413`; zeta `≤ 6` on `hFE` `:2421`;
  joint `0.04` on `hJoint` `:2430`.
* Product identity CLOSED with no premises: `cutR10_hProd_closed :486`.

Closeout verdict (honest inventory, no closure claimed beyond banked):
* CLOSED values re-banked below by direct call (10 items): Gamma half
  `1 / 2`; zeta `3` / `2` edges; reflected Euler `2`; FE sliver + full
  conditionals (exact open chi / middle premises restated); joint sliver
  `1072 / 5`; plus the three gap files restated exactly.
* OPEN gaps (exact Props, each a single explicit hypothesis):
  (G1) `hProdCap : ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100`;
  (G2) `hFE : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
    (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2`;
  (G3) `hJoint : ∀ z : ℂ,
    z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1) →
    ‖(1 / 2 : ℂ) * shiftedS z * (shiftedS z - 1)‖ *
    ‖((Real.pi : ℂ) ^ (-(shiftedS z / 2)))‖ *
    ‖Complex.Gamma (shiftedS z / 2)‖ * ‖zeta (shiftedS z)‖ ≤ (0.04 : ℝ)`.
  FE inner opens: `hChi : ‖cutR10_chiFE (1 - t)‖ ≤ 3` and
  `hMid : ∀ u : ℂ, (-0.50 : ℝ) ≤ u.re → u.re ≤ (2.06 : ℝ) →
    (-11.56 : ℝ) ≤ u.im → u.im ≤ (-8.44 : ℝ) → ‖zeta u‖ ≤ 2`.
* No new imports; nothing else touched; residual is exactly G1-G3.
-/

namespace Door3CutR10BallSup

/-- Gamma best half rechain11 (`≤ 1 / 2`). -/
theorem cutR10_gamma_best_half_rechain11 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ)) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 2 :=
  cutR10_gamma_best_half_rechain10 s hlo hhi hilo hihi

/-- Zeta edge rechain11 (`Re ≥ 3 / 2 → ≤ 3`). -/
theorem cutR10_zeta_edge_three_rechain11 (s : ℂ) (hs : 3 / 2 ≤ s.re) :
    ‖zeta s‖ ≤ 3 :=
  cutR10_zeta_edge_three_rechain10 s hs

/-- Zeta edge rechain11 (`Re ≥ 2 → ≤ 2`). -/
theorem cutR10_zeta_edge_two_rechain11 (s : ℂ) (hs : (2 : ℝ) ≤ s.re) :
    ‖zeta s‖ ≤ 2 :=
  cutR10_zeta_edge_two_rechain10 s hs

/-- Reflected Euler rechain11 (`t.re ≤ -1 → reflected ≤ 2`). -/
theorem cutR10_reflected_Euler_two_rechain11 (t : ℂ)
    (ht : t.re ≤ (-1 : ℝ)) :
    ‖zeta (1 - t)‖ ≤ 2 :=
  cutR10_reflected_Euler_two_rechain10 t ht

/-- Sliver hFE conditional rechain11: given chi `≤ 3`. -/
theorem cutR10_hFE_sliver_conditional_rechain11 (t : ℂ)
    (hlo : (-1.06 : ℝ) ≤ t.re) (hhi : t.re ≤ (-1 : ℝ))
    (hilo : (8.44 : ℝ) ≤ t.im) (hihi : t.im ≤ (11.56 : ℝ))
    (hChi : ‖cutR10_chiFE (1 - t)‖ ≤ 3) :
    ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_sliver_conditional_rechain10 t hlo hhi hilo hihi hChi

/-- Full hFE conditional rechain11: chi `≤ 3` + middle reflected `≤ 2`. -/
theorem cutR10_hFE_full_conditional_rechain11
    (hChi : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) →
      ‖cutR10_chiFE (1 - t)‖ ≤ 3)
    (hMid : ∀ u : ℂ, (-0.50 : ℝ) ≤ u.re → u.re ≤ (2.06 : ℝ) →
    ...
    ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_full_conditional_rechain10 hChi hMid

/-- Joint sliver rechain11 on in-ball `Re ≥ 2` (`≤ 1072 / 5`). -/
theorem cutR10_joint_sliver_rechain11 (z : ℂ)
    (hz : z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1))
    (hs2 : (2 : ℝ) ≤ (shiftedS z).re) :
    ‖xiShiftedEntire z‖ ≤ (1072 / 5 : ℝ) :=
  cutR10_joint_sliver_rechain10 z hz hs2

/-- Gamma gap11: `1 / 100` gated on single explicit `hProdCap`. -/
theorem cutR10_gamma_hundred_gap11 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hProdCap : ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100 :=
...
/-- Zeta gap11: full-rect `≤ 6` gated on single explicit `hFE`. -/
theorem cutR10_zeta_six_gap11 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hFE : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2) :
    ‖zeta s‖ ≤ 6 :=
  cutR10_zeta_six_gap10 s hlo hhi hilo hihi hFE

/-- Joint gap11: exact `0.04` gated on single explicit `hJoint`. -/
theorem cutR10_joint_exact_gap11
    (hJoint : ∀ z : ℂ, z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1) →
      ‖(1 / 2 : ℂ) * shiftedS z * (shiftedS z - 1)‖ *
        ‖((Real.pi : ℂ) ^ (-(shiftedS z / 2)))‖ *
        ‖Complex.Gamma (shiftedS z / 2)‖ * ‖zeta (shiftedS z)‖
    ...
    ∀ z : ℂ, z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1) →
      ‖xiShiftedEntire z‖ ≤ (0.04 : ℝ) :=
  cutR10_joint_exact_gap10 hJoint

end Door3CutR10BallSup

/-! # APPEND-12 (hProdCap honest attempt + triple gap file; append-only tail):

Grep record (read before writing, APPEND-11 tail :2442-2558):
* Gamma best half `≤ 1 / 2`: base `:1358`, rechain11 `:2481`.
* Zeta edges: `≤ 3` on `Re ≥ 3 / 2` base `:1073`, rechain11 `:2488`;
  `≤ 2` on `Re ≥ 2` base `:1640`, rechain11 `:2493`.
* Reflected Euler `≤ 2` on `t.re ≤ -1`: base `:1920`, rechain11 `:2498`.
* FE sliver conditional base `:1948`, rechain11 `:2504`
  (needs `‖cutR10_chiFE (1 - t)‖ ≤ 3`); FE full conditional base `:1962`,
  rechain11 `:2512` (needs chi `≤ 3` + middle reflected `≤ 2`).
* Joint sliver `≤ 1072 / 5` on in-ball `Re ≥ 2`: base `:2064`,
  rechain11 `:2524`.
* Gaps: Gamma `1 / 100` on `hProdCap` `:2531`; zeta `≤ 6` on `hFE` `:2539`;
  joint `0.04` on `hJoint` `:2548`.
* Product identity CLOSED with no premises: `cutR10_hProd_closed :486`.
* Shortfall record: `(1 / 2) / (1 / 100) = 50` at `:2018`.
* Banked-shape search in this file: `D3SG_tall_prod_bound_half` cited only
  as read-only shape at `:897`; every local `‖Gamma (s / 2)‖ ≤ 1 / 100`
  occurrence takes `hProdCap` as its premise (identity gate); no closed
  local `1 / 100` product cap exists.

Verdict here:
* hProdCap honest attempt: chain the banked half-rate product caps present
  locally (domination `:691` + shift `:743` + square product `:771` +
  real caps `:808` `:839` + denom lower `:871` landing at half `:1358`,
  rechained below by direct call through `:2481`). The `1 / 100` target is
  `50x` beyond banked, so it stays gated on the single explicit `hProdCap`
  filed exactly below. No closure beyond banked is claimed.
* Best banked zeta stays `3` / `2` on edges; best banked joint stays
  `1072 / 5` on the `Re ≥ 2` sliver (rechained below by direct call).
* Full-rect `1 / 100` stays gated on single explicit `hProdCap`;
  full-rect `≤ 6` stays gated on single explicit `hFE`;
  exact `0.04` stays gated on single explicit `hJoint`.
No new imports; nothing else touched.
-/

namespace Door3CutR10BallSup

/-- Gamma best banked half rechain12 (`≤ 1 / 2`). -/
theorem cutR10_gamma_best_half_rechain12 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ)) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 2 :=
  cutR10_gamma_best_half_rechain11 s hlo hhi hilo hihi

/-- Shortfall rechain12: `(1 / 2) / (1 / 100) = 50`. -/
theorem cutR10_gamma_half_to_hundred_shortfall_rechain12 :
    ((1 / 2 : ℝ) / (1 / 100) = 50) :=
  cutR10_gamma_half_to_hundred_shortfall

/-- Zeta edge rechain12 (`Re ≥ 3 / 2 → ≤ 3`). -/
theorem cutR10_zeta_edge_three_rechain12 (s : ℂ) (hs : 3 / 2 ≤ s.re) :
    ‖zeta s‖ ≤ 3 :=
  cutR10_zeta_edge_three_rechain11 s hs

/-- Zeta edge rechain12 (`Re ≥ 2 → ≤ 2`). -/
theorem cutR10_zeta_edge_two_rechain12 (s : ℂ) (hs : (2 : ℝ) ≤ s.re) :
    ‖zeta s‖ ≤ 2 :=
  cutR10_zeta_edge_two_rechain11 s hs

/-- Reflected Euler rechain12 (`t.re ≤ -1 → reflected ≤ 2`). -/
theorem cutR10_reflected_Euler_two_rechain12 (t : ℂ)
    (ht : t.re ≤ (-1 : ℝ)) :
    ‖zeta (1 - t)‖ ≤ 2 :=
  cutR10_reflected_Euler_two_rechain11 t ht

/-- Sliver hFE conditional rechain12: given chi `≤ 3`. -/
theorem cutR10_hFE_sliver_conditional_rechain12 (t : ℂ)
    (hlo : (-1.06 : ℝ) ≤ t.re) (hhi : t.re ≤ (-1 : ℝ))
    (hilo : (8.44 : ℝ) ≤ t.im) (hihi : t.im ≤ (11.56 : ℝ))
    (hChi : ‖cutR10_chiFE (1 - t)‖ ≤ 3) :
    ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_sliver_conditional_rechain11 t hlo hhi hilo hihi hChi

/-- Full hFE conditional rechain12: chi `≤ 3` + middle reflected `≤ 2`. -/
theorem cutR10_hFE_full_conditional_rechain12
    (hChi : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) →
      ‖cutR10_chiFE (1 - t)‖ ≤ 3)
    (hMid : ∀ u : ℂ, (-0.50 : ℝ) ≤ u.re → u.re ≤ (2.06 : ℝ) →
    ...
    ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_full_conditional_rechain11 hChi hMid

/-- Joint sliver rechain12 on in-ball `Re ≥ 2` (`≤ 1072 / 5`). -/
theorem cutR10_joint_sliver_rechain12 (z : ℂ)
    (hz : z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1))
    (hs2 : (2 : ℝ) ≤ (shiftedS z).re) :
    ‖xiShiftedEntire z‖ ≤ (1072 / 5 : ℝ) :=
  cutR10_joint_sliver_rechain11 z hz hs2

/-- Gamma gap12: `1 / 100` gated on single explicit `hProdCap`. -/
theorem cutR10_gamma_hundred_gap12 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hProdCap : ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100 :=
...
/-- Zeta gap12: full-rect `≤ 6` gated on single explicit `hFE`. -/
theorem cutR10_zeta_six_gap12 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hFE : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2) :
    ‖zeta s‖ ≤ 6 :=
  cutR10_zeta_six_gap11 s hlo hhi hilo hihi hFE

/-- Joint gap12: exact `0.04` gated on single explicit `hJoint`. -/
theorem cutR10_joint_exact_gap12
    (hJoint : ∀ z : ℂ, z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1) →
      ‖(1 / 2 : ℂ) * shiftedS z * (shiftedS z - 1)‖ *
        ‖((Real.pi : ℂ) ^ (-(shiftedS z / 2)))‖ *
        ‖Complex.Gamma (shiftedS z / 2)‖ * ‖zeta (shiftedS z)‖
    ...
    ∀ z : ℂ, z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1) →
      ‖xiShiftedEntire z‖ ≤ (0.04 : ℝ) :=
  cutR10_joint_exact_gap11 hJoint

end Door3CutR10BallSup

/-! # APPEND-13 (chi honest attempt + triple gap file; append-only tail):

Grep record (read before writing, APPEND-12 tail :2560-2681):
* Gamma best half `≤ 1 / 2`: base `:1358`, rechain12 `:2598`.
* Shortfall `(1 / 2) / (1 / 100) = 50`: base `:2018`, rechain12 `:2606`.
* Zeta edges: `≤ 3` on `Re ≥ 3 / 2` base `:1073`, rechain12 `:2610`;
  `≤ 2` on `Re ≥ 2` base `:1640`, rechain12 `:2615`.
* Reflected Euler `≤ 2` on `t.re ≤ -1`: base `:1920`, rechain12 `:2620`.
* FE sliver conditional base `:1948`, rechain12 `:2626`
  (needs `‖cutR10_chiFE (1 - t)‖ ≤ 3`); FE full conditional base `:1962`,
  rechain12 `:2634` (needs chi `≤ 3` + middle reflected `≤ 2`).
* Joint sliver `≤ 1072 / 5` on in-ball `Re ≥ 2`: base `:2064`,
  rechain12 `:2646`.
* Gaps: Gamma `1 / 100` on `hProdCap` `:2653`; zeta `≤ 6` on `hFE` `:2661`;
  joint `0.04` on `hJoint` `:2670`.
* Product identity CLOSED with no premises: `cutR10_hProd_closed :486`.
* Chi definition `cutR10_chiFE`: `:1886`; FE bridge
  `cutR10_zeta_of_chi_reflected` `:1930`; sliver `:1948`; full `:1962`.

Chi honest attempt (verified locally, no closure claimed):
* Searched this file for `cutR10_chiFE` upper caps at `≤ 3`: every
  occurrence takes an explicit `hChi : ‖cutR10_chiFE (1 - t)‖ ≤ 3`
  premise (identity gate) at `:1951`, `:1963`, `:2388`, `:2395`,
  `:2507`, `:2513`, `:2630`, `:2636`. No closed local
  `‖cutR10_chiFE _‖ ≤ 3` exists; banked chi numerals cited at `:1864`
  are exponential-scale and far above `3`. Hence the chi cap stays
  gated on the single explicit `hChi` filed exactly below as
  `cutR10_chi_three_gap13`.
* Best banked zeta stays `3` / `2` on edges; best banked Gamma stays
  `1 / 2` (50x short of `1 / 100`); best banked joint stays
  `1072 / 5` on the `Re ≥ 2` sliver (rechained below by direct call).
* Full-rect `1 / 100` stays gated on single explicit `hProdCap`;
  full-rect `≤ 6` stays gated on single explicit `hFE`;
  exact `0.04` stays gated on single explicit `hJoint`.
...
/-- Gamma best banked half rechain13 (`≤ 1 / 2`). -/
theorem cutR10_gamma_best_half_rechain13 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ)) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 2 :=
  cutR10_gamma_best_half_rechain12 s hlo hhi hilo hihi

/-- Shortfall rechain13: `(1 / 2) / (1 / 100) = 50`. -/
theorem cutR10_gamma_half_to_hundred_shortfall_rechain13 :
    ((1 / 2 : ℝ) / (1 / 100) = 50) :=
  cutR10_gamma_half_to_hundred_shortfall

/-- Zeta edge rechain13 (`Re ≥ 3 / 2 → ≤ 3`). -/
theorem cutR10_zeta_edge_three_rechain13 (s : ℂ) (hs : 3 / 2 ≤ s.re) :
    ‖zeta s‖ ≤ 3 :=
  cutR10_zeta_edge_three_rechain12 s hs

/-- Zeta edge rechain13 (`Re ≥ 2 → ≤ 2`). -/
theorem cutR10_zeta_edge_two_rechain13 (s : ℂ) (hs : (2 : ℝ) ≤ s.re) :
    ‖zeta s‖ ≤ 2 :=
  cutR10_zeta_edge_two_rechain12 s hs

/-- Reflected Euler rechain13 (`t.re ≤ -1 → reflected ≤ 2`). -/
theorem cutR10_reflected_Euler_two_rechain13 (t : ℂ)
    (ht : t.re ≤ (-1 : ℝ)) :
    ‖zeta (1 - t)‖ ≤ 2 :=
  cutR10_reflected_Euler_two_rechain12 t ht

/-- Sliver hFE conditional rechain13: given chi `≤ 3`. -/
theorem cutR10_hFE_sliver_conditional_rechain13 (t : ℂ)
    (hlo : (-1.06 : ℝ) ≤ t.re) (hhi : t.re ≤ (-1 : ℝ))
    (hilo : (8.44 : ℝ) ≤ t.im) (hihi : t.im ≤ (11.56 : ℝ))
    (hChi : ‖cutR10_chiFE (1 - t)‖ ≤ 3) :
    ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_sliver_conditional_rechain12 t hlo hhi hilo hihi hChi

/-- Full hFE conditional rechain13: chi `≤ 3` + middle reflected `≤ 2`. -/
theorem cutR10_hFE_full_conditional_rechain13
    (hChi : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) →
      ‖cutR10_chiFE (1 - t)‖ ≤ 3)
    (hMid : ∀ u : ℂ, (-0.50 : ℝ) ≤ u.re → u.re ≤ (2.06 : ℝ) →
    ...
    ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_full_conditional_rechain12 hChi hMid

/-- Joint sliver rechain13 on in-ball `Re ≥ 2` (`≤ 1072 / 5`). -/
theorem cutR10_joint_sliver_rechain13 (z : ℂ)
    (hz : z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1))
    (hs2 : (2 : ℝ) ≤ (shiftedS z).re) :
    ‖xiShiftedEntire z‖ ≤ (1072 / 5 : ℝ) :=
  cutR10_joint_sliver_rechain12 z hz hs2

/-- Chi gap13: `‖cutR10_chiFE (1 - t)‖ ≤ 3` gated on single explicit `hChi`.
No banked local closed cap at `≤ 3` exists (see header record); this files
the exact open premise for the FE route. -/
theorem cutR10_chi_three_gap13 (t : ℂ)
...
/-- Gamma gap13: `1 / 100` gated on single explicit `hProdCap`. -/
theorem cutR10_gamma_hundred_gap13 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hProdCap : ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100 :=
...
/-- Zeta gap13: full-rect `≤ 6` gated on single explicit `hFE`. -/
theorem cutR10_zeta_six_gap13 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hFE : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2) :
    ‖zeta s‖ ≤ 6 :=
  cutR10_zeta_six_gap12 s hlo hhi hilo hihi hFE

/-- Joint gap13: exact `0.04` gated on single explicit `hJoint`. -/
theorem cutR10_joint_exact_gap13
    (hJoint : ∀ z : ℂ, z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1) →
      ‖(1 / 2 : ℂ) * shiftedS z * (shiftedS z - 1)‖ *
        ‖((Real.pi : ℂ) ^ (-(shiftedS z / 2)))‖ *
        ‖Complex.Gamma (shiftedS z / 2)‖ * ‖zeta (shiftedS z)‖
    ...
    ∀ z : ℂ, z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1) →
      ‖xiShiftedEntire z‖ ≤ (0.04 : ℝ) :=
  cutR10_joint_exact_gap12 hJoint

end Door3CutR10BallSup

/-! # APPEND-14 (middle honest attempt + triple gap file; append-only tail):

Grep record (read before writing, APPEND-13 tail :2683-2815):
* Gamma best half `≤ 1 / 2`: rechain13 `:2723`.
* Shortfall `(1 / 2) / (1 / 100) = 50`: rechain13 `:2730`.
* Zeta edges: `≤ 3` on `Re ≥ 3 / 2` rechain13 `:2735`;
  `≤ 2` on `Re ≥ 2` rechain13 `:2740`.
* Reflected Euler `≤ 2` on `t.re ≤ -1`: rechain13 `:2745`.
* FE sliver conditional rechain13 `:2751`
  (needs `‖cutR10_chiFE (1 - t)‖ ≤ 3`); FE full conditional
  rechain13 `:2759` (needs chi `≤ 3` + middle reflected `≤ 2`).
* Joint sliver `≤ 1072 / 5` on in-ball `Re ≥ 2`: rechain13 `:2771`.
* Gaps: chi `≤ 3` on `hChi` `:2780`; Gamma `1 / 100` on `hProdCap` `:2788`;
  zeta `≤ 6` on `hFE` `:2796`; joint `0.04` on `hJoint` `:2805`.
* Product identity CLOSED with no premises: `cutR10_hProd_closed :486`.
* Chi definition `cutR10_chiFE`: `:1886`; FE bridge
  `cutR10_zeta_of_chi_reflected` `:1930`; sliver `:1948`; full `:1962`.
* Middle premise shape `hMid : ∀ u, -0.50 ≤ Re → ≤ 2.06 →
  -11.56 ≤ Im → ≤ -8.44 → ‖zeta u‖ ≤ 2` at `:1966`, `:2639`, `:2763`.

Middle honest attempt (verified locally, no closure claimed beyond banked):
* Searched this file for closed `‖zeta u‖ ≤ 2` on the full middle
  rectangle `[-0.50, 2.06] x [-11.56, -8.44]`: every occurrence takes an
  explicit universal `hMid` premise (identity gate) at `:1966`, `:2398`,
  `:2516`, `:2639`, `:2763`. No closed local cap on the full rectangle
  exists; Euler banked caps cover only `Re ≥ 3 / 2 → ≤ 3` (`:1073`)
  and `Re ≥ 2 → ≤ 2` (`:1640`).
* Honest banked chain therefore closes only the `Re ≥ 2` sub-strip
  (width `0.06` of `2.56`): `cutR10_middle_edge_two_closed14` below by
  direct call through `cutR10_zeta_edge_two_rechain13`.
* Remainder `Re < 2` stays open; the full `hMid` shape stays gated on the
  single explicit universal premise filed exactly below as
...
/-- Gamma best banked half rechain14 (`≤ 1 / 2`). -/
theorem cutR10_gamma_best_half_rechain14 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ)) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 2 :=
  cutR10_gamma_best_half_rechain13 s hlo hhi hilo hihi

/-- Shortfall rechain14: `(1 / 2) / (1 / 100) = 50`. -/
theorem cutR10_gamma_half_to_hundred_shortfall_rechain14 :
    ((1 / 2 : ℝ) / (1 / 100) = 50) :=
  cutR10_gamma_half_to_hundred_shortfall

/-- Zeta edge rechain14 (`Re ≥ 3 / 2 → ≤ 3`). -/
theorem cutR10_zeta_edge_three_rechain14 (s : ℂ) (hs : 3 / 2 ≤ s.re) :
    ‖zeta s‖ ≤ 3 :=
  cutR10_zeta_edge_three_rechain13 s hs

/-- Zeta edge rechain14 (`Re ≥ 2 → ≤ 2`). -/
theorem cutR10_zeta_edge_two_rechain14 (s : ℂ) (hs : (2 : ℝ) ≤ s.re) :
    ‖zeta s‖ ≤ 2 :=
  cutR10_zeta_edge_two_rechain13 s hs

/-- Reflected Euler rechain14 (`t.re ≤ -1 → reflected ≤ 2`). -/
theorem cutR10_reflected_Euler_two_rechain14 (t : ℂ)
    (ht : t.re ≤ (-1 : ℝ)) :
    ‖zeta (1 - t)‖ ≤ 2 :=
  cutR10_reflected_Euler_two_rechain13 t ht

/-- Sliver hFE conditional rechain14: given chi `≤ 3`. -/
theorem cutR10_hFE_sliver_conditional_rechain14 (t : ℂ)
    (hlo : (-1.06 : ℝ) ≤ t.re) (hhi : t.re ≤ (-1 : ℝ))
    (hilo : (8.44 : ℝ) ≤ t.im) (hihi : t.im ≤ (11.56 : ℝ))
    (hChi : ‖cutR10_chiFE (1 - t)‖ ≤ 3) :
    ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_sliver_conditional_rechain13 t hlo hhi hilo hihi hChi

/-- Full hFE conditional rechain14: chi `≤ 3` + middle reflected `≤ 2`. -/
theorem cutR10_hFE_full_conditional_rechain14
    (hChi : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) →
      ‖cutR10_chiFE (1 - t)‖ ≤ 3)
    (hMid : ∀ u : ℂ, (-0.50 : ℝ) ≤ u.re → u.re ≤ (2.06 : ℝ) →
    ...
    ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_full_conditional_rechain13 hChi hMid

/-- Joint sliver rechain14 on in-ball `Re ≥ 2` (`≤ 1072 / 5`). -/
theorem cutR10_joint_sliver_rechain14 (z : ℂ)
    (hz : z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1))
    (hs2 : (2 : ℝ) ≤ (shiftedS z).re) :
    ‖xiShiftedEntire z‖ ≤ (1072 / 5 : ℝ) :=
  cutR10_joint_sliver_rechain13 z hz hs2

/-- Middle partial CLOSED: `‖zeta u‖ ≤ 2` on the `Re ≥ 2` sub-strip of the
middle rectangle, by direct call to the banked edge-two cap. This is the
honest banked reach for `hMid`; the `Re < 2` remainder stays open. -/
theorem cutR10_middle_edge_two_closed14 (u : ℂ)
    (hlo : (-0.50 : ℝ) ≤ u.re) (hhi : u.re ≤ (2.06 : ℝ))
    (hilo : (-11.56 : ℝ) ≤ u.im) (hihi : u.im ≤ (-8.44 : ℝ))
    (hs2 : (2 : ℝ) ≤ u.re) :
    ‖zeta u‖ ≤ 2 :=
  cutR10_zeta_edge_two_rechain13 u hs2

/-- Middle gap14: full `hMid` shape gated on the single explicit universal
premise. No banked local closed cap on `Re < 2` exists; this files the
exact open remainder. -/
theorem cutR10_middle_two_gap14
    (hMid : ∀ u : ℂ, (-0.50 : ℝ) ≤ u.re → u.re ≤ (2.06 : ℝ) →
      (-11.56 : ℝ) ≤ u.im → u.im ≤ (-8.44 : ℝ) → ‖zeta u‖ ≤ 2) :
...
/-- Chi gap14: `‖cutR10_chiFE (1 - t)‖ ≤ 3` gated on single explicit `hChi`. -/
theorem cutR10_chi_three_gap14 (t : ℂ)
    (hlo : (-1.06 : ℝ) ≤ t.re) (hhi : t.re ≤ (3 / 2 : ℝ))
    (hilo : (8.44 : ℝ) ≤ t.im) (hihi : t.im ≤ (11.56 : ℝ))
    (hChi : ‖cutR10_chiFE (1 - t)‖ ≤ 3) :
    ‖cutR10_chiFE (1 - t)‖ ≤ 3 :=
...
/-- Gamma gap14: `1 / 100` gated on single explicit `hProdCap`. -/
theorem cutR10_gamma_hundred_gap14 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hProdCap : ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100 :=
...
/-- Zeta gap14: full-rect `≤ 6` gated on single explicit `hFE`. -/
theorem cutR10_zeta_six_gap14 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hFE : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2) :
    ‖zeta s‖ ≤ 6 :=
  cutR10_zeta_six_gap13 s hlo hhi hilo hihi hFE

/-- Joint gap14: exact `0.04` gated on single explicit `hJoint`. -/
theorem cutR10_joint_exact_gap14
    (hJoint : ∀ z : ℂ, z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1) →
      ‖(1 / 2 : ℂ) * shiftedS z * (shiftedS z - 1)‖ *
        ‖((Real.pi : ℂ) ^ (-(shiftedS z / 2)))‖ *
        ‖Complex.Gamma (shiftedS z / 2)‖ * ‖zeta (shiftedS z)‖
...
/-! # APPEND-16 (FINAL LEDGER + banked rechain; append-only tail):

Grep record (read before writing, tail :2817-3025):
* APPEND-14 banked: gamma half `≤ 1 / 2` `:2862`; shortfall `= 50` `:2869`;
  zeta three `:2874`, zeta two `:2879`, reflected `:2884`;
  FE sliver conditional `:2890` (needs chi `≤ 3`);
  FE full conditional `:2898` (needs chi `≤ 3` + mid `≤ 2`);
  joint sliver `≤ 1072 / 5` on in-ball `Re ≥ 2` `:2910`;
  middle edge `≤ 2` on `Re ≥ 2` `:2919`;
  chi gap `:2937`, gamma gap `:2945`, zeta gap `:2953`, joint gap `:2962`.
* APPEND-15 banked: middle edge closed15 `:3009` (via closed14);
  middle gap15 `:3018` (via gap14).
* No closed local cap exists for: full-rect Gamma `1 / 100`,
  ...
  (identity gate) at `:1966`, `:2400`, `:2518`, `:2641`, `:2765`,
  `:2904`, `:2931`, `:3020`.
...
CLOSED values (exact numerals, no premises beyond stated bounds):
  ...
  C18 shortfall `(1/2)/(1/100) = 50` (`:2018`, rechained `:2869`).
  C19 middle edge `‖zeta u‖ ≤ 2` on `Re ≥ 2` sub-strip (`:2919`, `:3009`).
...
width `0.06` of `2.56`).
Blocked-token scan for APPEND-16 range: zero new hits; placeholder-free
with explicit binders only. No new imports; nothing else touched.
-/

namespace Door3CutR10BallSup

/-- Gamma best half final16 (`≤ 1 / 2`). -/
theorem cutR10_gamma_best_half_final16 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ)) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 2 :=
  cutR10_gamma_best_half_rechain14 s hlo hhi hilo hihi

/-- Shortfall final16: `(1 / 2) / (1 / 100) = 50`. -/
theorem cutR10_gamma_half_to_hundred_shortfall_final16 :
    ((1 / 2 : ℝ) / (1 / 100) = 50) :=
  cutR10_gamma_half_to_hundred_shortfall_rechain14

/-- Zeta edge final16 (`Re ≥ 3 / 2 → ≤ 3`). -/
theorem cutR10_zeta_edge_three_final16 (s : ℂ) (hs : 3 / 2 ≤ s.re) :
    ‖zeta s‖ ≤ 3 :=
  cutR10_zeta_edge_three_rechain14 s hs

/-- Zeta edge final16 (`Re ≥ 2 → ≤ 2`). -/
theorem cutR10_zeta_edge_two_final16 (s : ℂ) (hs : (2 : ℝ) ≤ s.re) :
    ‖zeta s‖ ≤ 2 :=
  cutR10_zeta_edge_two_rechain14 s hs

/-- Reflected Euler final16 (`t.re ≤ -1 → reflected ≤ 2`). -/
theorem cutR10_reflected_Euler_two_final16 (t : ℂ)
    (ht : t.re ≤ (-1 : ℝ)) :
    ‖zeta (1 - t)‖ ≤ 2 :=
  cutR10_reflected_Euler_two_rechain14 t ht

/-- Sliver FE conditional final16: given chi `≤ 3`. -/
theorem cutR10_hFE_sliver_conditional_final16 (t : ℂ)
    (hlo : (-1.06 : ℝ) ≤ t.re) (hhi : t.re ≤ (-1 : ℝ))
    (hilo : (8.44 : ℝ) ≤ t.im) (hihi : t.im ≤ (11.56 : ℝ))
    (hChi : ‖cutR10_chiFE (1 - t)‖ ≤ 3) :
    ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_sliver_conditional_rechain14 t hlo hhi hilo hihi hChi

/-- Full FE conditional final16: chi `≤ 3` + middle reflected `≤ 2`. -/
theorem cutR10_hFE_full_conditional_final16
    (hChi : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) →
      ‖cutR10_chiFE (1 - t)‖ ≤ 3)
    (hMid : ∀ u : ℂ, (-0.50 : ℝ) ≤ u.re → u.re ≤ (2.06 : ℝ) →
    ...
    ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2 :=
  cutR10_hFE_full_conditional_rechain14 hChi hMid

/-- Joint sliver final16 on in-ball `Re ≥ 2` (`≤ 1072 / 5`). -/
theorem cutR10_joint_sliver_final16 (z : ℂ)
    (hz : z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1))
    (hs2 : (2 : ℝ) ≤ (shiftedS z).re) :
    ‖xiShiftedEntire z‖ ≤ (1072 / 5 : ℝ) :=
  cutR10_joint_sliver_rechain14 z hz hs2

/-- Middle partial CLOSED final16 (`≤ 2` on `Re ≥ 2` sub-strip). -/
theorem cutR10_middle_edge_two_final16 (u : ℂ)
    (hlo : (-0.50 : ℝ) ≤ u.re) (hhi : u.re ≤ (2.06 : ℝ))
    (hilo : (-11.56 : ℝ) ≤ u.im) (hihi : u.im ≤ (-8.44 : ℝ))
    (hs2 : (2 : ℝ) ≤ u.re) :
    ‖zeta u‖ ≤ 2 :=
  cutR10_middle_edge_two_closed15 u hlo hhi hilo hihi hs2

/-- Middle gap final16: full `hMid` shape on explicit premise. -/
theorem cutR10_middle_two_gap_final16
    (hMid : ∀ u : ℂ, (-0.50 : ℝ) ≤ u.re → u.re ≤ (2.06 : ℝ) →
      (-11.56 : ℝ) ≤ u.im → u.im ≤ (-8.44 : ℝ) → ‖zeta u‖ ≤ 2) :
    ∀ u : ℂ, (-0.50 : ℝ) ≤ u.re → u.re ≤ (2.06 : ℝ) →
      (-11.56 : ℝ) ≤ u.im → u.im ≤ (-8.44 : ℝ) → ‖zeta u‖ ≤ 2 :=
  cutR10_middle_two_gap15 hMid

/-- Chi gap final16 (`≤ 3` on explicit `hChi`). -/
theorem cutR10_chi_three_gap_final16 (t : ℂ)
    (hlo : (-1.06 : ℝ) ≤ t.re) (hhi : t.re ≤ (3 / 2 : ℝ))
    (hilo : (8.44 : ℝ) ≤ t.im) (hihi : t.im ≤ (11.56 : ℝ))
    (hChi : ‖cutR10_chiFE (1 - t)‖ ≤ 3) :
    ‖cutR10_chiFE (1 - t)‖ ≤ 3 :=
  cutR10_chi_three_gap14 t hlo hhi hilo hihi hChi

/-- Gamma gap final16 (`1 / 100` on explicit `hProdCap`). -/
theorem cutR10_gamma_hundred_gap_final16 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hProdCap : ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100) :
    ‖Complex.Gamma (s / 2)‖ ≤ 1 / 100 :=
  cutR10_gamma_hundred_gap14 s hlo hhi hilo hihi hProdCap

/-- Zeta gap final16 (full-rect `≤ 6` on explicit `hFE`). -/
theorem cutR10_zeta_six_gap_final16 (s : ℂ)
    (hlo : (-1.06 : ℝ) ≤ s.re) (hhi : s.re ≤ (2.06 : ℝ))
    (hilo : (8.44 : ℝ) ≤ s.im) (hihi : s.im ≤ (11.56 : ℝ))
    (hFE : ∀ t : ℂ, (-1.06 : ℝ) ≤ t.re → t.re ≤ (3 / 2 : ℝ) →
      (8.44 : ℝ) ≤ t.im → t.im ≤ (11.56 : ℝ) → ‖zeta t‖ ≤ 3 * 2) :
    ‖zeta s‖ ≤ 6 :=
  cutR10_zeta_six_gap14 s hlo hhi hilo hihi hFE

/-- Joint gap final16 (exact `0.04` on explicit `hJoint`). -/
theorem cutR10_joint_exact_gap_final16
    (hJoint : ∀ z : ℂ, z ∈ Metric.closedBall CutR10.center (CutR10.radius + 1) →
      ‖(1 / 2 : ℂ) * shiftedS z * (shiftedS z - 1)‖ *
        ‖((Real.pi : ℂ) ^ (-(shiftedS z / 2)))‖ *
        ‖Complex.Gamma (shiftedS z / 2)‖ * ‖zeta (shiftedS z)‖
