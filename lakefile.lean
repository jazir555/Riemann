      ...
      `KadiriZeroFree, `KadiriOrderInfra, `KadiriZerosInfra2, `KadiriHScratch,
       ...
       `ktest, `ScratchCheck, `JTest, `XiStub, `TestScratch, `TestZeta0,
         ...
         `KadiriHadamardAffine, `central_cover_assembly, `central_cover_trusted, `interval_arith,
          `door3_cell_checker, `door3_deriv_certs, `door3_gamma_product, `door3_gamma_cutoff, `door3_zeta_cutoff, `door3_tail_eta_upper, `door3_boundary_real, `door3_boundary_endpoints, `door3_top_edge, `door3_imag_axis_strip, `door3_height11_bridge, `door3_float_bridge_audit, `door3_real_radius_bridge, `door3_zeta_pair_tightening, `door3_grid_boundary_counterexample, `door3_trusted_statement_audit, `door3_closed_cover, `door3_zeta_cert_range_audit, `door3_numeric_targets, `door3_rational_certificates, `door3_s4_imag, `door3_center_bounds, `door3_rational_grid_cover, `door3_real_center_bounds, `door3_off_axis_certificates, `door3_stirling_gamma, `door3_rh_wiring, `door3_cutR10_ballsup, `door3_sliver_nonvan, `door3_cutL10_remainders, `door3_first_cell, `door3_sliver_edge, `door3_srect_zeta, `door3_cell_suppliers, `door3_cells_batchA, `door3_cells_batchB, `door3_cells_batchC, `door3_cells_batchD, `door3_tierB_subdiv, `door3_cutR10_retier, `door3_gamma_pi2, `door3_tail_edge_push, `door3_cutoff_rouche, `door3_dp_trig, `door3_dp_terms, `door3_hmain_bridge, `door3_cells_batchE, `door3_premise_poly, `door3_premise_gamma, `door3_premise_tier,  `door3_premise_pi,
`door3_premise_zeta, `door3_dp_headA, `door3_tier_direct, `door3_dp_headB, `door3_zlowN, `door3_gamma_low, `door3_dp_headC1, `door3_subdiv_plan, `door3_zeta_N64, `door3_gamma_disc, `door3_R05_close, `door3_deriv_up, `door3_gamma_real_low, `door3_gamma_feed, `door3_digamma, `door3_eta_prime, `door3_stirling_rem, `door3_psi_slope, `door3_complex_wendel, `door3_R02_ball_advance, `door3_pilot_R00_zeta, `door3_R02_zeta_bridge]
  globs := #[Glob.one `ZeroFreeRegionHadamard, Glob.one `ZeroFreeRegionProof, Glob.one `riemann_hypothesis, Glob.one `HadamardBridge, Glob.submodules `Zeta23, Glob.one `KadiriZeroFree, Glob.one `KadiriOrderInfra, Glob.one `KadiriZerosInfra2, Glob.one `KadiriHScratch, Glob.one `KadiriDigammaBound, Glob.one `ZeroFreeRegion, Glob.one `ZeroFreeRegionInfra, Glob.one `JensenTranslation, Glob.one `JensenScratch,         Glob.one `riemann_hypothesis_newsection, Glob.one `cross_door_synthesis, Glob.one `rh_residual_gap, Glob.one `rh_certificate, Glob.one `rh_certificate_infra, Glob.one `rh_analytic_infra, Glob.one `rh_infra, Glob.one `rh_term_fps, Glob.one `rh_term_deriv,         Glob.one `rh_zeta_cert_data,         Glob.one `rh_zeta_cert_central, Glob.one `float_xi_cover, Glob.one `float_xi_approx,         Glob.one `float_zeta, Glob.one `float_xi_bridge, Glob.one `float_real_bridge, Glob.one `float_jensen, Glob.one `zeta_rigorous, Glob.one `MertensEstimate, Glob.one `SorryFix, Glob.one `ApproxZetaLowerBound, Glob.one `BorelCaratheodory, Glob.one `ktest, Glob.one `ScratchCheck, Glob.one `JTest, Glob.one `XiStub, Glob.one `TestScratch, Glob.one `TestZeta0, Glob.one `XiZerosInfiniteScratch, Glob.one `OrderXiScratch, Glob.one `TailLaguerreScratch, Glob.one `FirstQuadrantScratch,
         Glob.one `KadiriHadamardAffine, Glob.one `central_cover_assembly, Glob.one `central_cover_trusted, Glob.one `interval_arith,
           Glob.one `door3_cell_checker, Glob.one `door3_deriv_certs, Glob.one `door3_gamma_product, Glob.one `door3_gamma_cutoff, Glob.one `door3_zeta_cutoff, Glob.one `door3_tail_eta_upper, Glob.one `door3_boundary_real, Glob.one `door3_boundary_endpoints, Glob.one `door3_top_edge, Glob.one `door3_imag_axis_strip, Glob.one `door3_height11_bridge, Glob.one `door3_float_bridge_audit, Glob.one `door3_real_radius_bridge, Glob.one `door3_zeta_pair_tightening, Glob.one `door3_grid_boundary_counterexample, Glob.one `door3_trusted_statement_audit, Glob.one `door3_closed_cover, Glob.one `door3_zeta_cert_range_audit, Glob.one `door3_numeric_targets, Glob.one `door3_rational_certificates, Glob.one `door3_s4_imag, Glob.one `door3_center_bounds, Glob.one `door3_rational_grid_cover, Glob.one `door3_real_center_bounds, Glob.one `door3_off_axis_certificates, Glob.one `door3_stirling_gamma, Glob.one `door3_rh_wiring, Glob.one `door3_cutR10_ballsup, Glob.one `door3_sliver_nonvan, Glob.one `door3_cutL10_remainders, Glob.one `door3_first_cell, Glob.one `door3_sliver_edge, Glob.one `door3_srect_zeta, Glob.one `door3_cell_suppliers, Glob.one `door3_cells_batchA, Glob.one `door3_cells_batchB, Glob.one `door3_cells_batchC, Glob.one `door3_cells_batchD, Glob.one `door3_tierB_subdiv, Glob.one `door3_cutR10_retier, Glob.one `door3_gamma_pi2, Glob.one `door3_tail_edge_push, Glob.one `door3_cutoff_rouche, Glob.one `door3_dp_trig, Glob.one `door3_dp_terms, Glob.one `door3_hmain_bridge, Glob.one `door3_cells_batchE, Glob.one `door3_premise_poly, Glob.one `door3_premise_gamma, Glob.one `door3_premise_tier, Glob.one `door3_premise_pi, Glob.one `door3_premise_zeta, Glob.one `door3_dp_headA, Glob.one `door3_tier_direct, Glob.one `door3_dp_headB, Glob.one `door3_zlowN, Glob.one `door3_gamma_low, Glob.one `door3_dp_headC1, Glob.one `door3_subdiv_plan, Glob.one `door3_zeta_N64, Glob.one `door3_gamma_disc, Glob.one `door3_R05_close, Glob.one `door3_deriv_up, Glob.one `door3_gamma_real_low, Glob.one `door3_gamma_feed, Glob.one `door3_digamma, Glob.one `door3_eta_prime, Glob.one `door3_stirling_rem, Glob.one `door3_psi_slope, Glob.one `door3_complex_wendel, Glob.one `door3_R02_ball_advance, Glob.one `door3_pilot_R00_zeta, Glob.one `door3_R02_zeta_bridge]
