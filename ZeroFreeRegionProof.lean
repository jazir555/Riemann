...
open Complex Real Topology ZeroFreeRegion
...
private theorem continuousOn_of_continuous {α E : Type*} [TopologicalSpace α]
    ...
    ContinuousOn f s := by
  intro x _hx
  exact Filter.Tendsto.mono_left (Continuous.continuousAt _hf) nhdsWithin_le_nhds
...
theorem riemannZeta_ne_zero_of_zeroFreeEdge
    ...
    -- Route A: factorization of −ζ'/ζ (equivalently L(Λ,·)) in terms of non-trivial zeros
    (σ : ℝ) (hσgt : 1 < σ)
    (Z : Finset ℂ) (hZmem : s ∈ Z)
    (hZre : ∀ ρ ∈ Z, 0 < ρ.re) (hZre_lt : ∀ ρ ∈ Z, ρ.re < 1)
    (analytic : ℂ → ℂ)
    (h_decomp : ∀ (s' : ℂ) (hs' : 1 < s'.re),
      LSeries ↗Λ s' = analytic s' - ∑ ρ' ∈ Z, (1 / (s' - ρ') + 1 / ρ'))
    (A₀ A₁ A₂ : ℝ) (hA₀ : 0 ≤ A₀) (hA₁ : 0 ≤ A₁)
    (h_analytic : 3 * (analytic (↑σ : ℂ)).re + 4 * (analytic (↑σ + ↑s.im * I)).re
        + (analytic (↑σ + 2 * ↑s.im * I)).re ≤ A₀ / (σ - 1) + A₁ * Real.log (|s.im| + 2) + A₂)
    (hRHS_pos : 0 < A₀ / (σ - 1) + A₁ * Real.log (|s.im| + 2) + A₂)
    (h_c : kadiriConstant / Real.log (|s.im| + 10) <
        4 / (A₀ / (σ - 1) + A₁ * Real.log (|s.im| + 2) + A₂) - (σ - 1)) :
    riemannZeta s ≠ 0 := by
  by_cases h1 : 1 ≤ s.re
  · exact riemannZeta_ne_zero_of_one_le_re h1
  · push Not at h1; intro hz
    have hρ₀re_pos : 0 < s.re := by
      have := zeroFreeEdge_gt_nine_tenths s.im ht
      linarith [hσ]
    have hρ₀re_lt : s.re < 1 := h1
    have hedge := zeroFreeEdge_from_factorization
      σ s.im hσgt Z s hZmem rfl hρ₀re_pos hρ₀re_lt
      hZre hZre_lt analytic h_decomp A₀ A₁ A₂ hA₀ hA₁ h_analytic hRHS_pos
    -- hedge : σ - s.re ≥ 4/(A₀/(σ-1) + A₁·log(|s.im|+2) + A₂)
    -- Rewrite: σ - s.re = (σ - 1) + (1 - s.re), so
    -- (σ - 1) + (1 - s.re) ≥ 4/RHS
    -- Combined with h_c: 4/RHS - (σ-1) > kadiriConstant/log(|s.im|+10),
    -- gives 1 - s.re > kadiriConstant/log(|s.im|+10).
    -- But hσ : s.re ≥ 1 - kadiriConstant/log(|s.im|+10), i.e. 1 - s.re ≤ kadiriConstant/log(|s.im|+10).
    -- Contradiction.
    have hσ' : s.re ≥ 1 - kadiriConstant / Real.log (|s.im| + 10) := by rwa [zeroFreeEdge] at hσ
    have h_RHS : A₀ / (σ - 1) + A₁ * Real.log (|s.im| + 2) + A₂ = A₀ / (σ - 1) + A₁ * Real.log (|s.im| + 2) + A₂ := rfl
    have : (σ - 1) + (1 - s.re) ≥ 4 / (A₀ / (σ - 1) + A₁ * Real.log (|s.im| + 2) + A₂) := by
      rw [show (σ - 1) + (1 - s.re) = σ - s.re from by ring]; exact hedge
    linarith
