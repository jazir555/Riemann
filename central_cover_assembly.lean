...
/-! ## ASSEMBLY-R01 leaf+H residual pair (append-only, no redefinition)

Grep (read-only, verified before writing):
- R01 leaf: `R01_leaf_obligations` at 1288-1290 =
  `(0.05 + 0.07 * R01.radius <= norm at R01.center) AND
   (forall w, R01.mem w -> ||deriv xiShifted w|| <= 0.07)`.
- R01 H: `R01_H_instance` at 1326-1336, membership-free (`c` + `hc_eq` only,
  no `hc_mem`), conclusion shape `∃ R ε M, ... ∧ 0 < ε ∧ deriv bound ∧ budget`.
- R00 residual shapes mirrored: `R00_center_of_tenth_lower` 16988,
  `R00_center_residual_tenth` 16993,
  `R00_center_obligation_of_residual_tenth` 16996,
  `R00_deriv_residual` 17019, `R00_leaf_of_residuals` 17022,
  `R00_H_of_residuals` 17026.
- R00D audit at 17039-17076 left untouched: it records the R00-deriv banked
  gap (`0.05 < 67200`) and introduces no new R00 obligation; nothing below
  re-proves or duplicates that audit.

Mirror for R01 mid tier `(ε, M) = (0.05, 0.07)`:
- `R01_budget_lt_app` lifts `R01_radius_lt` + `fine_feasible_mid` to the true
  radius, exactly as `R00_budget_lt_app` does for R00.
- Center residual `0.2 <= norm at R01.center` discharges the center conjunct
  via the budget.
- `R01_deriv_residual` is exactly the `deriv_bound` conjunct.
- `R01_leaf_of_residuals` rebuilds `R01_leaf_obligations` from the two residuals.
- `R01_H_of_residuals` discharges the membership-free H shape via `R01_H_instance`.
Value: full R01 H leaf now conditional only on two named numeric enclosures.
Gap: center needs `0.2 <= norm` enclosure at R01 center; deriv needs uniform
  `||deriv xiShifted|| <= 0.07` enclosure on `R01` (not closed here).
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R01 mid tier on the true radius. -/
theorem R01_budget_lt_app : (0.05 : ℝ) + 0.07 * R01.radius < 0.2 := by
  have h := R01_radius_lt
  have hM : 0.07 * R01.radius < 0.07 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R01_center_of_two_tenths_lower (hC : (0.2 : ℝ) ≤ ‖xiShifted R01.center‖) :
    (0.05 : ℝ) + 0.07 * R01.radius ≤ ‖xiShifted R01.center‖ := by
  have hB := R01_budget_lt_app
  linarith

def R01_center_residual_two_tenths : Prop :=
  (0.2 : ℝ) ≤ ‖xiShifted R01.center‖

theorem R01_center_obligation_of_residual_two_tenths
    (h : R01_center_residual_two_tenths) :
    (0.05 : ℝ) + 0.07 * R01.radius ≤ ‖xiShifted R01.center‖ :=
  R01_center_of_two_tenths_lower h

def R01_deriv_residual : Prop :=
  ∀ w, R01.mem w → ‖deriv xiShifted w‖ ≤ (0.07 : ℝ)

theorem R01_leaf_of_residuals (hc : R01_center_residual_two_tenths)
    (hd : R01_deriv_residual) : R01_leaf_obligations :=
  ⟨R01_center_obligation_of_residual_two_tenths hc, hd⟩

theorem R01_H_of_residuals (hc : R01_center_residual_two_tenths)
    (hd : R01_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ)
    (hc_eq : c = (-7.5, -5, 0.01, 0.2)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
...
/-! ## ASSEMBLY-R02 leaf+H residual set (append-only, no redefinition)

Grep record (read-only, verified before writing):
- `R02_leaf_obligations` 1470-1472: center `(0.002 + 0.07 * R02.radius <= norm
  at R02.center)` AND deriv `(forall w, R02.mem w -> ||deriv xiShifted w|| <= 0.07)`
  (outer tier `(ε, M) = (0.002, 0.07)`).
- `R02_H_instance` 1499-1509: takes `(h : R02_leaf_obligations) (c) (hc_mem :
  c ∈ gridFine) (hc_eq : c = (-8, -5.5, 0.01, 0.2))`, conclusion shape
  `∃ R ε M, ... ∧ 0 < ε ∧ deriv bound ∧ budget`.
- `R02_radius_lt` 1457 (`R02.radius < 1.26`); `R02_mem_gridFine` 1460.
- R01 shapes mirrored (`:17111-17148`): `R01_budget_lt_app` 17111,
  `R01_center_of_two_tenths_lower` 17117, `R01_center_residual_two_tenths` 17122,
  `R01_center_obligation_of_residual_two_tenths` 17125, `R01_deriv_residual` 17130,
  `R01_leaf_of_residuals` 17133, `R01_H_of_residuals` 17137 (membership-free
  `c + hc_eq` only; R02 keeps `hc_mem + hc_eq` per `:1499`).
- R00SUP working sup angle reused, not duplicated: `R00_deriv_tight_sup` 17188,
  `R00_tight_div` 17193, `R00_deriv_of_tight_sup` 17197, `R00_ball2_sup_tight`
  17209, `R00_closedBall_radius_subset_ball2` 17214, `R00_tight_of_ball2` 17223,
  `R00_deriv_of_ball2` 17229, `R00_H_of_tight_sup` 17238, `R00_H_of_ball2` 17250.
  No new sup def below.
- Grep-clean before writing: no `R02_budget_lt_app`, `R02_center_of_tenth_lower`,
  `R02_center_residual_tenth`, `R02_center_obligation_of_residual_tenth`,
  `R02_deriv_residual`, `R02_leaf_of_residuals`, `R02_H_of_residuals` in file.

Mirror for R02 outer tier `(ε, M) = (0.002, 0.07)` at `c02 = (-8, -5.5, 0.01, 0.2)`:
budget `0.002 + 0.07 * radius < 0.1` lifts `R02_radius_lt`, so
`0.1 <= norm at R02.center` discharges the center conjunct; deriv residual is
exactly the second conjunct; `leaf_of` rebuilds `R02_leaf_obligations`;
`H_of` discharges the gridFine H shape via `R02_H_instance`.
Value-or-gap: budget `0.002 + 0.07 * 1.26 = 0.0902 < 0.1`; full R02 H leaf now
conditional only on two named numeric enclosures (`0.1 <= norm` at center +
uniform `||deriv xiShifted|| <= 0.07` on `R02`, not closed here).
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R02 outer tier on the true radius. -/
theorem R02_budget_lt_app : (0.002 : ℝ) + 0.07 * R02.radius < 0.1 := by
  have h := R02_radius_lt
  have hM : 0.07 * R02.radius < 0.07 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R02_center_of_tenth_lower (hC : (0.1 : ℝ) ≤ ‖xiShifted R02.center‖) :
    (0.002 : ℝ) + 0.07 * R02.radius ≤ ‖xiShifted R02.center‖ := by
  have hB := R02_budget_lt_app
  linarith

def R02_center_residual_tenth : Prop :=
  (0.1 : ℝ) ≤ ‖xiShifted R02.center‖

theorem R02_center_obligation_of_residual_tenth (h : R02_center_residual_tenth) :
    (0.002 : ℝ) + 0.07 * R02.radius ≤ ‖xiShifted R02.center‖ :=
  R02_center_of_tenth_lower h

def R02_deriv_residual : Prop :=
  ∀ w, R02.mem w → ‖deriv xiShifted w‖ ≤ (0.07 : ℝ)

theorem R02_leaf_of_residuals (hc : R02_center_residual_tenth)
    (hd : R02_deriv_residual) : R02_leaf_obligations :=
  ⟨R02_center_obligation_of_residual_tenth hc, hd⟩

theorem R02_H_of_residuals (hc : R02_center_residual_tenth)
    (hd : R02_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (-8, -5.5, 0.01, 0.2)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
      ...
      0 < ε ∧ (∀ w, R.mem w → ‖deriv xiShifted w‖ ≤ M) ∧
      ε + M * R.radius ≤ ‖xiShifted R.center‖ :=
  R02_H_instance (R02_leaf_of_residuals hc hd) c hc_mem hc_eq

end CentralCoverAssembly

/-! ## ASSEMBLY-R01 deriv tight-sup conditional (append-only, no redefinition)

Grep record (read-only, verified before writing):
- `R01_leaf_obligations` 1288-1290: center `(0.05 + 0.07 * R01.radius <= norm)`
  AND deriv `(forall w, R01.mem w -> ||deriv xiShifted w|| <= 0.07)` (mid tier).
- `R01_H_instance` 1326-1336: takes `(h) (c) (hc_eq : c = (-7.5,-5,0.01,0.2))`,
  membership-free (no `hc_mem`); conclusion `Exists R e M, ... /\ 0 < e /\ deriv bound /\ budget`.
- `R01_radius_lt` 1280 (`R01.radius < 1.26`); `R01_strip_lo/hi` 1264-1265.
- R01 residuals reused, not redefined: `R01_budget_lt_app` 17111,
  `R01_center_of_two_tenths_lower` 17117, `R01_center_residual_two_tenths` 17122,
  `R01_center_obligation_of_residual_two_tenths` 17125, `R01_deriv_residual` 17130,
  `R01_leaf_of_residuals` 17133, `R01_H_of_residuals` 17137.
- R00SUP chain reused, not duplicated: `R00_strip_of_mem` 17180,
  `R00_deriv_tight_sup` 17188, `R00_tight_div` 17193, `R00_deriv_of_tight_sup` 17197,
  `R00_ball2_sup_tight` 17209, `R00_closedBall_radius_subset_ball2` 17214,
  `R00_tight_of_ball2` 17223, `R00_deriv_of_ball2` 17229,
  `R00_H_of_tight_sup` 17238, `R00_H_of_ball2` 17250 (C=0.0125/r=0.25 to 0.05).
- Banked Cauchy: `uniform_deriv_of_closedBall_bound` 6233 (`M = C / r`).
- R02 set untouched (other worker owns it): `R02_budget_lt_app` 17300 through
  `R02_H_of_residuals` 17325; no R02 names below.
- Grep-clean before writing: no `R01_strip_of_mem`, `R01_deriv_tight_sup`,
  `R01_tight_div`, `R01_deriv_of_tight_sup`, `R01_ball2_sup_tight`,
  `R01_closedBall_radius_subset_ball2`, `R01_tight_of_ball2`,
  `R01_deriv_of_ball2`, `R01_H_of_tight_sup`, `R01_H_of_ball2` in file.

Mirror for R01 mid tier `(e, M) = (0.05, 0.07)` at `c01 = (-7.5,-5,0.01,0.2)`:
tighten `C` to `0.0175` at same `r = 0.25` (`0.0175 / 0.25 = 0.07`), plus a
`ball 2` variant (`radius + 0.25 < 2` via `R01_radius_lt`). No new obligation
beyond the two sup Props below; exact residual `R01_deriv_residual` stands
if the sups stay open.
Value-or-gap: budget `0.05 + 0.07 * 1.26 = 0.1382 < 0.2` (via `R01_budget_lt_app`);
tight `0.0175` meets the `0.07` tier exactly (`0.0175 / 0.25 = 0.07`).
-/

namespace CentralCoverAssembly

/-- Every `R01` point lies strictly inside the strip. -/
theorem R01_strip_of_mem {w : ℂ} (hw : R01.mem w) :
    -(1 / 2 : ℝ) < w.im ∧ w.im < (1 / 2 : ℝ) := by
  obtain ⟨_, _, hy0, hy1⟩ := hw
  have hLo : -(1 / 2 : ℝ) < R01.y0 := R01_strip_lo
  have hHi : R01.y1 < (1 / 2 : ℝ) := R01_strip_hi
  constructor <;> linarith

/-- Tight sup premise on the `premDeriv` ball shape: `0.0175` for the `0.07` tier. -/
def R01_deriv_tight_sup : Prop :=
  ∀ z ∈ Metric.closedBall R01.center (R01.radius + 0.25),
    ‖xiShiftedEntire z‖ ≤ (0.0175 : ℝ)

/-- Closed-form threshold: `0.0175 / 0.25 = 0.07`. -/
theorem R01_tight_div : (0.0175 : ℝ) / 0.25 = 0.07 := by
  norm_num

/-- Tight sup discharges the exact deriv residual via `:6233` at `r = 0.25`. -/
theorem R01_deriv_of_tight_sup (hBall : R01_deriv_tight_sup) :
    R01_deriv_residual := by
  intro w hw
  have hStrip : ∀ u, R01.mem u → -(1 / 2 : ℝ) < u.im ∧ u.im < (1 / 2 : ℝ) :=
    fun u hu => R01_strip_of_mem hu
  have h := DerivCauchyBridge.uniform_deriv_of_closedBall_bound
    R01 0.25 0.0175 (by norm_num) hStrip hBall w hw
  have heq : (0.0175 : ℝ) / 0.25 = 0.07 := R01_tight_div
  rw [heq] at h
  exact h

/-- `ball R01c 2` variant: same `0.0175` bound on `closedBall R01.center 2`. -/
def R01_ball2_sup_tight : Prop :=
  ∀ z ∈ Metric.closedBall R01.center 2,
    ‖xiShiftedEntire z‖ ≤ (0.0175 : ℝ)

/-- The `premDeriv` ball sits inside the `ball 2` (`radius < 1.26`). -/
theorem R01_closedBall_radius_subset_ball2 :
...
theorem R01_tight_of_ball2 (h : R01_ball2_sup_tight) :
    R01_deriv_tight_sup := by
  intro z hz
  exact h z (R01_closedBall_radius_subset_ball2 hz)

/-- `ball 2` sup discharges the exact deriv residual. -/
theorem R01_deriv_of_ball2 (h : R01_ball2_sup_tight) :
    R01_deriv_residual :=
  R01_deriv_of_tight_sup (R01_tight_of_ball2 h)

/-- Tight sup + center residual discharge the `H` leaf shape at `c01`
(membership-free, via `R01_H_instance`). -/
theorem R01_H_of_tight_sup (hc : R01_center_residual_two_tenths)
    (hBall : R01_deriv_tight_sup)
    (c : ℝ × ℝ × ℝ × ℝ)
    (hc_eq : c = (-7.5, -5, 0.01, 0.2)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
      ...
      0 < ε ∧ (∀ w, R.mem w → ‖deriv xiShifted w‖ ≤ M) ∧
      ε + M * R.radius ≤ ‖xiShifted R.center‖ :=
  R01_H_of_residuals hc (R01_deriv_of_tight_sup hBall) c hc_eq

/-- `ball 2` sup + center residual discharge the `H` leaf shape at `c01`
(membership-free, via `R01_H_instance`). -/
theorem R01_H_of_ball2 (hc : R01_center_residual_two_tenths)
    (hBall : R01_ball2_sup_tight)
    (c : ℝ × ℝ × ℝ × ℝ)
    (hc_eq : c = (-7.5, -5, 0.01, 0.2)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
      ...
      0 < ε ∧ (∀ w, R.mem w → ‖deriv xiShifted w‖ ≤ M) ∧
      ε + M * R.radius ≤ ‖xiShifted R.center‖ :=
  R01_H_of_residuals hc (R01_deriv_of_ball2 hBall) c hc_eq

end CentralCoverAssembly

/-! ## ASSEMBLY-R03 leaf+H residual set (append-only, no redefinition)

Grep record (read-only, verified before writing):
- `R03_leaf_obligations` 1556-1558: center `(0.05 + 0.07 * R03.radius <= norm
  at R03.center)` AND deriv `(forall w, R03.mem w -> ||deriv xiShifted w|| <= 0.07)`
  (mid tier `(ε, M) = (0.05, 0.07)`).
- `R03_H_instance` 1585-1595: takes `(h : R03_leaf_obligations) (c)
  (hc_mem : c ∈ gridFine) (hc_eq : c = (-6, -3.5, 0.01, 0.2))`, conclusion
  `∃ R ε M, ... ∧ 0 < ε ∧ deriv bound ∧ budget`.
- `R03_radius_lt` 1543-1544 (`R03.radius < 1.26`); `R03_mem_gridFine` 1546-1552.
- R01 shapes mirrored (`17111-17146`): `R01_budget_lt_app` 17111,
  `R01_center_of_two_tenths_lower` 17117, `R01_center_residual_two_tenths` 17122,
  `R01_center_obligation_of_residual_two_tenths` 17125, `R01_deriv_residual` 17130,
  `R01_leaf_of_residuals` 17133, `R01_H_of_residuals` 17137.
- R02 shapes left untouched (other worker owns it): `R02_budget_lt_app` 17300
  through `R02_H_of_residuals` 17325; no R02 names below.
- Grep-clean before writing: no `R03_budget_lt_app`, `R03_center_of_two_tenths_lower`,
  `R03_center_residual_two_tenths`, `R03_center_obligation_of_residual_two_tenths`,
  `R03_deriv_residual`, `R03_leaf_of_residuals`, `R03_H_of_residuals` in file.

Mirror for R03 mid tier `(ε, M) = (0.05, 0.07)` at `c03 = (-6, -3.5, 0.01, 0.2)`:
budget `0.05 + 0.07 * radius < 0.2` lifts `R03_radius_lt`, so
`0.2 <= norm at R03.center` discharges the center conjunct; deriv residual is
exactly the second conjunct; `leaf_of` rebuilds `R03_leaf_obligations`;
`H_of` discharges the gridFine H shape via `R03_H_instance`
(with `hc_mem + hc_eq` per `1585`, matching R02 form).
Value-or-gap: budget `0.05 + 0.07 * 1.26 = 0.1382 < 0.2`; full R03 H leaf now
conditional only on two named numeric enclosures (`0.2 <= norm` at center +
uniform `||deriv xiShifted|| <= 0.07` on `R03`, not closed here).
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R03 mid tier on the true radius. -/
theorem R03_budget_lt_app : (0.05 : ℝ) + 0.07 * R03.radius < 0.2 := by
  have h := R03_radius_lt
  have hM : 0.07 * R03.radius < 0.07 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R03_center_of_two_tenths_lower (hC : (0.2 : ℝ) ≤ ‖xiShifted R03.center‖) :
    (0.05 : ℝ) + 0.07 * R03.radius ≤ ‖xiShifted R03.center‖ := by
  have hB := R03_budget_lt_app
  linarith

def R03_center_residual_two_tenths : Prop :=
  (0.2 : ℝ) ≤ ‖xiShifted R03.center‖

theorem R03_center_obligation_of_residual_two_tenths
    (h : R03_center_residual_two_tenths) :
    (0.05 : ℝ) + 0.07 * R03.radius ≤ ‖xiShifted R03.center‖ :=
  R03_center_of_two_tenths_lower h

def R03_deriv_residual : Prop :=
  ∀ w, R03.mem w → ‖deriv xiShifted w‖ ≤ (0.07 : ℝ)

theorem R03_leaf_of_residuals (hc : R03_center_residual_two_tenths)
    (hd : R03_deriv_residual) : R03_leaf_obligations :=
  ⟨R03_center_obligation_of_residual_two_tenths hc, hd⟩

theorem R03_H_of_residuals (hc : R03_center_residual_two_tenths)
    (hd : R03_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (-6, -3.5, 0.01, 0.2)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
      ...
      0 < ε ∧ (∀ w, R.mem w → ‖deriv xiShifted w‖ ≤ M) ∧
      ε + M * R.radius ≤ ‖xiShifted R.center‖ :=
  R03_H_instance (R03_leaf_of_residuals hc hd) c hc_mem hc_eq

end CentralCoverAssembly

/-! ## ASSEMBLY-R04 leaf+H residual set (append-only, no redefinition)

Grep record (read-only, verified before writing):
- `R04_leaf_obligations` 1642-1644: center `(0.05 + 0.07 * R04.radius <= norm
  at R04.center)` AND deriv `(forall w, R04.mem w -> ||deriv xiShifted w|| <= 0.07)`
  (mid tier `(eps, M) = (0.05, 0.07)`).
- `R04_H_instance` 1671-1681: takes `(h : R04_leaf_obligations) (c)
  (hc_mem : c in gridFine) (hc_eq : c = (-4, -1.5, 0.01, 0.2))`, conclusion
  `Exists R eps M, ... /\ 0 < eps /\ deriv bound /\ budget`.
- `R04_radius_lt` 1629-1630 (`R04.radius < 1.26`); `R04_mem_gridFine` 1632-1638.
- R03 shapes mirrored (`17490-17525`): `R03_budget_lt_app` 17490,
  `R03_center_of_two_tenths_lower` 17496, `R03_center_residual_two_tenths` 17501,
  `R03_center_obligation_of_residual_two_tenths` 17504, `R03_deriv_residual` 17509,
  `R03_leaf_of_residuals` 17512, `R03_H_of_residuals` 17516.
- Grep-clean before writing: no `R04_budget_lt_app`, `R04_center_of_two_tenths_lower`,
  `R04_center_residual_two_tenths`, `R04_center_obligation_of_residual_two_tenths`,
  `R04_deriv_residual`, `R04_leaf_of_residuals`, `R04_H_of_residuals` in file.

Mirror for R04 mid tier `(eps, M) = (0.05, 0.07)` at `c04 = (-4, -1.5, 0.01, 0.2)`:
budget `0.05 + 0.07 * radius < 0.2` lifts `R04_radius_lt`, so
`0.2 <= norm at R04.center` discharges the center conjunct; deriv residual is
exactly the second conjunct; `leaf_of` rebuilds `R04_leaf_obligations`;
`H_of` discharges the gridFine H shape via `R04_H_instance`
(with `hc_mem + hc_eq` per `1671`, matching R03 form).
Value-or-gap: budget `0.05 + 0.07 * 1.26 = 0.1382 < 0.2`; full R04 H leaf now
conditional only on two named numeric enclosures (`0.2 <= norm` at center +
uniform `||deriv xiShifted|| <= 0.07` on `R04`, not closed here).
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R04 mid tier on the true radius. -/
theorem R04_budget_lt_app : (0.05 : ℝ) + 0.07 * R04.radius < 0.2 := by
  have h := R04_radius_lt
  have hM : 0.07 * R04.radius < 0.07 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R04_center_of_two_tenths_lower (hC : (0.2 : ℝ) ≤ ‖xiShifted R04.center‖) :
    (0.05 : ℝ) + 0.07 * R04.radius ≤ ‖xiShifted R04.center‖ := by
  have hB := R04_budget_lt_app
  linarith

def R04_center_residual_two_tenths : Prop :=
  (0.2 : ℝ) ≤ ‖xiShifted R04.center‖

theorem R04_center_obligation_of_residual_two_tenths
    (h : R04_center_residual_two_tenths) :
    (0.05 : ℝ) + 0.07 * R04.radius ≤ ‖xiShifted R04.center‖ :=
  R04_center_of_two_tenths_lower h

def R04_deriv_residual : Prop :=
  ∀ w, R04.mem w → ‖deriv xiShifted w‖ ≤ (0.07 : ℝ)

theorem R04_leaf_of_residuals (hc : R04_center_residual_two_tenths)
    (hd : R04_deriv_residual) : R04_leaf_obligations :=
  ⟨R04_center_obligation_of_residual_two_tenths hc, hd⟩

theorem R04_H_of_residuals (hc : R04_center_residual_two_tenths)
    (hd : R04_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (-4, -1.5, 0.01, 0.2)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
      ...
      0 < ε ∧ (∀ w, R.mem w → ‖deriv xiShifted w‖ ≤ M) ∧
      ε + M * R.radius ≤ ‖xiShifted R.center‖ :=
  R04_H_instance (R04_leaf_of_residuals hc hd) c hc_mem hc_eq

end CentralCoverAssembly

/-! ## ASSEMBLY-R05 leaf+H residual set (append-only, no redefinition)

Grep record (read-only, verified before writing):
- `R05_leaf_obligations` 1728-1730: center `(0.15 + 0.06 * R05.radius <= norm
  at R05.center)` AND deriv `(forall w, R05.mem w -> ||deriv xiShifted w|| <= 0.06)`
  (inner tier `(eps, M) = (0.15, 0.06)`).
- `R05_H_instance` 1757-1767: takes `(h : R05_leaf_obligations) (c)
  (hc_mem : c in gridFine) (hc_eq : c = (-2, 0.5, 0.01, 0.2))`, conclusion
  `Exists R eps M, ... /\ 0 < eps /\ deriv bound /\ budget`.
- `R05_radius_lt` 1715-1716 (`R05.radius < 1.26`); `R05_mem_gridFine` 1718-1724.
- `fine_feasible_inner` 1029 (`0.15 + 0.06 * 1.26 < 0.3`); `fine_eps_inner_pos` 1034.
- R04 shapes mirrored (`17562-17598`): `R04_budget_lt_app` 17563,
  `R04_center_of_two_tenths_lower` 17569, `R04_center_residual_two_tenths` 17574,
  `R04_center_obligation_of_residual_two_tenths` 17577, `R04_deriv_residual` 17582,
  `R04_leaf_of_residuals` 17585, `R04_H_of_residuals` 17589.
- Grep-clean before writing: no `R05_budget_lt_app`, `R05_center_of_three_tenths_lower`,
  `R05_center_residual_three_tenths`, `R05_center_obligation_of_residual_three_tenths`,
  `R05_deriv_residual`, `R05_leaf_of_residuals`, `R05_H_of_residuals` in file.

Mirror for R05 inner tier `(eps, M) = (0.15, 0.06)` at `c05 = (-2, 0.5, 0.01, 0.2)`:
budget `0.15 + 0.06 * radius < 0.3` lifts `R05_radius_lt`, so
`0.3 <= norm at R05.center` discharges the center conjunct; deriv residual is
exactly the second conjunct; `leaf_of` rebuilds `R05_leaf_obligations`;
`H_of` discharges the gridFine H shape via `R05_H_instance`
(with `hc_mem + hc_eq` per `1757`, matching R04 form).
Value-or-gap: budget `0.15 + 0.06 * 1.26 = 0.2256 < 0.3`; full R05 H leaf now
conditional only on two named numeric enclosures (`0.3 <= norm` at center +
uniform `||deriv xiShifted|| <= 0.06` on `R05`, not closed here).
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R05 inner tier on the true radius. -/
theorem R05_budget_lt_app : (0.15 : ℝ) + 0.06 * R05.radius < 0.3 := by
  have h := R05_radius_lt
  have hM : 0.06 * R05.radius < 0.06 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R05_center_of_three_tenths_lower (hC : (0.3 : ℝ) ≤ ‖xiShifted R05.center‖) :
    (0.15 : ℝ) + 0.06 * R05.radius ≤ ‖xiShifted R05.center‖ := by
  have hB := R05_budget_lt_app
  linarith

def R05_center_residual_three_tenths : Prop :=
  (0.3 : ℝ) ≤ ‖xiShifted R05.center‖

theorem R05_center_obligation_of_residual_three_tenths
    (h : R05_center_residual_three_tenths) :
    (0.15 : ℝ) + 0.06 * R05.radius ≤ ‖xiShifted R05.center‖ :=
  R05_center_of_three_tenths_lower h

def R05_deriv_residual : Prop :=
  ∀ w, R05.mem w → ‖deriv xiShifted w‖ ≤ (0.06 : ℝ)

theorem R05_leaf_of_residuals (hc : R05_center_residual_three_tenths)
    (hd : R05_deriv_residual) : R05_leaf_obligations :=
  ⟨R05_center_obligation_of_residual_three_tenths hc, hd⟩

theorem R05_H_of_residuals (hc : R05_center_residual_three_tenths)
    (hd : R05_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (-2, 0.5, 0.01, 0.2)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
      ...
      0 < ε ∧ (∀ w, R.mem w → ‖deriv xiShifted w‖ ≤ M) ∧
      ε + M * R.radius ≤ ‖xiShifted R.center‖ :=
  R05_H_instance (R05_leaf_of_residuals hc hd) c hc_mem hc_eq

end CentralCoverAssembly

/-! ## ASSEMBLY-R07 leaf+H residual set (append-only, no redefinition)

Grep record (read-only, verified before writing):
- `R07_leaf_obligations` 1900-1902: center `(0.05 + 0.07 * R07.radius <= norm
  at R07.center)` AND deriv `(forall w, R07.mem w -> ||deriv xiShifted w|| <= 0.07)`
  (mid tier `(eps, M) = (0.05, 0.07)`).
- `R07_H_instance` 1929-1939: takes `(h : R07_leaf_obligations) (c)
  (hc_mem : c in gridFine) (hc_eq : c = (2, 4.5, 0.01, 0.2))`, conclusion
  `Exists R eps M, ... /\ 0 < eps /\ deriv bound /\ budget`.
- `R07_radius_lt` 1887-1888 (`R07.radius < 1.26`); `R07_mem_gridFine` 1890-1896.
- `fine_feasible_mid` 1028 (`0.05 + 0.07 * 1.26 < 0.2`); `fine_eps_mid_pos` 1033.
- R05 shapes mirrored (`17634-17670`): `R05_budget_lt_app` 17635,
  `R05_center_of_three_tenths_lower` 17641, `R05_center_residual_three_tenths` 17646,
  `R05_center_obligation_of_residual_three_tenths` 17649, `R05_deriv_residual` 17654,
  `R05_leaf_of_residuals` 17657, `R05_H_of_residuals` 17661 (inner tier
  `(0.15, 0.06)` at `c = (-2, 0.5, 0.01, 0.2)` with `hc_mem + hc_eq`).
- R06 base left untouched (other cell owns it): `R06` 1774 through
  `R06_H_instance` 1843-1853; no R06 names below.
- Grep-clean before writing: no `R07_budget_lt_app`, `R07_center_of_two_tenths_lower`,
  `R07_center_residual_two_tenths`, `R07_center_obligation_of_residual_two_tenths`,
  `R07_deriv_residual`, `R07_leaf_of_residuals`, `R07_H_of_residuals` in file.

Mirror for R07 mid tier `(eps, M) = (0.05, 0.07)` at `c07 = (2, 4.5, 0.01, 0.2)`:
budget `0.05 + 0.07 * radius < 0.2` lifts `R07_radius_lt`, so
`0.2 <= norm at R07.center` discharges the center conjunct; deriv residual is
exactly the second conjunct; `leaf_of` rebuilds `R07_leaf_obligations`;
`H_of` discharges the gridFine H shape via `R07_H_instance`
(with `hc_mem + hc_eq` per `1929`, matching R05 form).
Value-or-gap: budget `0.05 + 0.07 * 1.26 = 0.1382 < 0.2`; full R07 H leaf now
conditional only on two named numeric enclosures (`0.2 <= norm` at center +
uniform `||deriv xiShifted|| <= 0.07` on `R07`, not closed here).
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R07 mid tier on the true radius. -/
theorem R07_budget_lt_app : (0.05 : ℝ) + 0.07 * R07.radius < 0.2 := by
  have h := R07_radius_lt
  have hM : 0.07 * R07.radius < 0.07 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R07_center_of_two_tenths_lower (hC : (0.2 : ℝ) ≤ ‖xiShifted R07.center‖) :
    (0.05 : ℝ) + 0.07 * R07.radius ≤ ‖xiShifted R07.center‖ := by
  have hB := R07_budget_lt_app
  linarith

def R07_center_residual_two_tenths : Prop :=
  (0.2 : ℝ) ≤ ‖xiShifted R07.center‖

theorem R07_center_obligation_of_residual_two_tenths
    (h : R07_center_residual_two_tenths) :
    (0.05 : ℝ) + 0.07 * R07.radius ≤ ‖xiShifted R07.center‖ :=
  R07_center_of_two_tenths_lower h

def R07_deriv_residual : Prop :=
  ∀ w, R07.mem w → ‖deriv xiShifted w‖ ≤ (0.07 : ℝ)

theorem R07_leaf_of_residuals (hc : R07_center_residual_two_tenths)
    (hd : R07_deriv_residual) : R07_leaf_obligations :=
  ⟨R07_center_obligation_of_residual_two_tenths hc, hd⟩

theorem R07_H_of_residuals (hc : R07_center_residual_two_tenths)
    (hd : R07_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (2, 4.5, 0.01, 0.2)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
...
/-! ## ASSEMBLY-R09 center+deriv residual set (one cell, mirrors R08)

- Banked in-file: `R09` 2032-2033 (`(6, 8.5, 0.01, 0.2)`, outer tier
  `(0.002, 0.07)` per leaf); `R09_radius_lt` 2059-2060
  (`R09.radius < 1.26` via `sample_cell_radius_bound`);
  `R09_mem_gridFine` 2062-2068; `R09_leaf_obligations` 2072-2074 (center
  `(0.002 + 0.07 * R09.radius <= norm)` AND deriv
  `(forall w, R09.mem w -> ||deriv xiShifted w|| <= 0.07)`);
  `R09_H_instance` 2101-2111 (takes `(h : R09_leaf_obligations) (c)`
  `(hc_mem : c ∈ gridFine)` `(hc_eq : c = (6, 8.5, 0.01, 0.2))`,
  gridFine form with `hc_mem + hc_eq`).
- R08 shapes mirrored (`17781-17817`): `R08_budget_lt_app` 17782,
  `R08_center_of_two_tenths_lower` 17788, `R08_center_residual_two_tenths`
  17793, `R08_center_obligation_of_residual_two_tenths` 17796,
  `R08_deriv_residual` 17801, `R08_leaf_of_residuals` 17804,
  `R08_H_of_residuals` 17808 (mid tier `(0.05, 0.07)` at
  `c = (4, 6.5, 0.01, 0.2)` with `hc_mem + hc_eq`).
- Grep-clean before writing: no `R09_budget_lt_app`,
  `R09_center_of_tenth_lower`, `R09_center_residual_tenth`,
  `R09_center_obligation_of_residual_tenth`, `R09_deriv_residual`,
  `R09_leaf_of_residuals`, `R09_H_of_residuals` in file; R00-R08 sets left
  untouched (no duplication).

Mirror for R09 outer tier `(eps, M) = (0.002, 0.07)` at
`c09 = (6, 8.5, 0.01, 0.2)`: budget `0.002 + 0.07 * radius < 0.1` lifts
`R09_radius_lt` (same `mul_lt_mul_of_pos_left` + `linarith` shape as R08),
so `0.1 <= norm at R09.center` discharges the center conjunct; deriv
residual is exactly the second conjunct; `leaf_of` rebuilds
`R09_leaf_obligations`; `H_of` discharges the gridFine H shape via
`R09_H_instance` (with `hc_mem + hc_eq` per `2101`, matching R08 form).
Value-or-gap: budget `0.002 + 0.07 * 1.26 = 0.0902 < 0.1`; full R09 H leaf
now conditional only on two named numeric enclosures (`0.1 <= norm` at
center + uniform `‖deriv xiShifted‖ <= 0.07` on `R09`, not closed here).
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R09 outer tier on the true radius. -/
theorem R09_budget_lt_app : (0.002 : ℝ) + 0.07 * R09.radius < 0.1 := by
  have h := R09_radius_lt
  have hM : 0.07 * R09.radius < 0.07 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R09_center_of_tenth_lower (hC : (0.1 : ℝ) ≤ ‖xiShifted R09.center‖) :
    (0.002 : ℝ) + 0.07 * R09.radius ≤ ‖xiShifted R09.center‖ := by
  have hB := R09_budget_lt_app
  linarith

def R09_center_residual_tenth : Prop :=
  (0.1 : ℝ) ≤ ‖xiShifted R09.center‖

theorem R09_center_obligation_of_residual_tenth (h : R09_center_residual_tenth) :
    (0.002 : ℝ) + 0.07 * R09.radius ≤ ‖xiShifted R09.center‖ :=
  R09_center_of_tenth_lower h

def R09_deriv_residual : Prop :=
  ∀ w, R09.mem w → ‖deriv xiShifted w‖ ≤ (0.07 : ℝ)

theorem R09_leaf_of_residuals (hc : R09_center_residual_tenth)
    (hd : R09_deriv_residual) : R09_leaf_obligations :=
  ⟨R09_center_obligation_of_residual_tenth hc, hd⟩

theorem R09_H_of_residuals (hc : R09_center_residual_tenth)
    (hd : R09_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (6, 8.5, 0.01, 0.2)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
...
/-! ## ASSEMBLY-R06 leaf+H residual set (append-only, no redefinition)

Grep record (read-only, verified before writing):
- `R06` 1774-1775 (`(0, 2.5, 0.01, 0.2)`, inner tier `(0.15, 0.06)`).
- `R06_radius_lt` 1801-1802 (`R06.radius < 1.26` via `sample_cell_radius_bound`).
- `R06_mem_gridFine` 1804-1810; `R06_leaf_obligations` 1814-1816 (center
  `(0.15 + 0.06 * R06.radius <= norm)` AND deriv
  `(forall w, R06.mem w -> ||deriv xiShifted w|| <= 0.06)`).
- `R06_H_instance` 1843-1853 (takes `(h : R06_leaf_obligations) (c)`
  `(hc_mem : c ∈ gridFine)` `(hc_eq : c = (0, 2.5, 0.01, 0.2))`,
  gridFine form with `hc_mem + hc_eq`).
- R05 shapes mirrored (`17635-17670`): `R05_budget_lt_app` 17635,
  `R05_center_of_three_tenths_lower` 17641,
  ...
  `R05_H_of_residuals` 17661 (inner tier `(0.15, 0.06)` at
  `c = (-2, 0.5, 0.01, 0.2)` with `hc_mem + hc_eq`).
- Grep-clean before writing: no `R06_budget_lt_app`,
  `R06_center_of_three_tenths_lower`, `R06_center_residual_three_tenths`,
  `R06_center_obligation_of_residual_three_tenths`, `R06_deriv_residual`,
  `R06_leaf_of_residuals`, `R06_H_of_residuals` in file; R00-R05/R07-R10 sets
  left untouched (no duplication).

Mirror for R06 inner tier `(eps, M) = (0.15, 0.06)` at
`c06 = (0, 2.5, 0.01, 0.2)`: budget `0.15 + 0.06 * radius < 0.3` lifts
`R06_radius_lt` (same `mul_lt_mul_of_pos_left` + `linarith` shape as R05),
so `0.3 <= norm at R06.center` discharges the center conjunct; deriv
residual is exactly the second conjunct; `leaf_of` rebuilds
`R06_leaf_obligations`; `H_of` discharges the gridFine H shape via
`R06_H_instance` (with `hc_mem + hc_eq` per `1843`, matching R05 form).
Value-or-gap: budget `0.15 + 0.06 * 1.26 = 0.2256 < 0.3`; full R06 H leaf
now conditional only on two named numeric enclosures (`0.3 <= norm` at
center + uniform `‖deriv xiShifted‖ <= 0.06` on `R06`, not closed here).
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R06 inner tier on the true radius. -/
theorem R06_budget_lt_app : (0.15 : ℝ) + 0.06 * R06.radius < 0.3 := by
  have h := R06_radius_lt
  have hM : 0.06 * R06.radius < 0.06 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R06_center_of_three_tenths_lower (hC : (0.3 : ℝ) ≤ ‖xiShifted R06.center‖) :
    (0.15 : ℝ) + 0.06 * R06.radius ≤ ‖xiShifted R06.center‖ := by
  have hB := R06_budget_lt_app
  linarith

def R06_center_residual_three_tenths : Prop :=
  (0.3 : ℝ) ≤ ‖xiShifted R06.center‖

theorem R06_center_obligation_of_residual_three_tenths
    (h : R06_center_residual_three_tenths) :
    (0.15 : ℝ) + 0.06 * R06.radius ≤ ‖xiShifted R06.center‖ :=
  R06_center_of_three_tenths_lower h

def R06_deriv_residual : Prop :=
  ∀ w, R06.mem w → ‖deriv xiShifted w‖ ≤ (0.06 : ℝ)

theorem R06_leaf_of_residuals (hc : R06_center_residual_three_tenths)
    (hd : R06_deriv_residual) : R06_leaf_obligations :=
  ⟨R06_center_obligation_of_residual_three_tenths hc, hd⟩

theorem R06_H_of_residuals (hc : R06_center_residual_three_tenths)
    (hd : R06_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (0, 2.5, 0.01, 0.2)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
      ...
      0 < ε ∧ (∀ w, R.mem w → ‖deriv xiShifted w‖ ≤ M) ∧
      ε + M * R.radius ≤ ‖xiShifted R.center‖ :=
  R06_H_instance (R06_leaf_of_residuals hc hd) c hc_mem hc_eq

end CentralCoverAssembly

/-! ## ASSEMBLY-R11 leaf+H residual set (append-only, no redefinition)

Grep record (read-only, verified before writing):
- `R11_radius_lt` 2520-2521 (`R11.radius < 1.26` via
  `sample_cell_radius_01_bound`); `R11_mem_gridFine` 2523-2529
  (`(-10, -7.5, 0.1, 0.3)`).
- `R11_leaf_obligations` 2533-2535 (center
  `(0.002 + 0.05 * R11.radius <= norm)` AND deriv
  `(forall w, R11.mem w -> ||deriv xiShifted w|| <= 0.05)`, outer tier
  `(0.002, 0.05)`).
- `R11_H_instance` 2562-2572 (takes `(h : R11_leaf_obligations) (c)`
  `(hc_mem : c ∈ gridFine)` `(hc_eq : c = (-10, -7.5, 0.1, 0.3))`,
  gridFine form with `hc_mem + hc_eq`).
- R10 shapes mirrored (`17917-17951`): `R10_budget_lt_app` 17917,
  `R10_center_of_tenth_lower` 17923, `R10_center_residual_tenth` 17928,
  `R10_center_obligation_of_residual_tenth` 17931, `R10_deriv_residual`
  17935, `R10_leaf_of_residuals` 17938, `R10_H_of_residuals` 17942
  (outer tier `(0.002, 0.05)` at `c = (7.5, 10, 0.01, 0.2)` with
  `hc_mem + hc_eq`).
- Grep-clean before writing: no `R11_budget_lt_app`,
  `R11_center_of_tenth_lower`, `R11_center_residual_tenth`,
  `R11_center_obligation_of_residual_tenth`, `R11_deriv_residual`,
  `R11_leaf_of_residuals`, `R11_H_of_residuals` in file; R00-R10 sets left
  untouched (no duplication).

Mirror for R11 outer tier `(eps, M) = (0.002, 0.05)` at
`c11 = (-10, -7.5, 0.1, 0.3)`: budget `0.002 + 0.05 * radius < 0.1` lifts
`R11_radius_lt` (same `mul_lt_mul_of_pos_left` + `linarith` shape as R10),
so `0.1 <= norm at R11.center` discharges the center conjunct; deriv
residual is exactly the second conjunct; `leaf_of` rebuilds
`R11_leaf_obligations`; `H_of` discharges the gridFine H shape via
`R11_H_instance` (with `hc_mem + hc_eq` per `2562`, matching R10 form).
Value-or-gap: budget `0.002 + 0.05 * 1.26 = 0.065 < 0.1`; full R11 H leaf
now conditional only on two named numeric enclosures (`0.1 <= norm` at
center + uniform `‖deriv xiShifted‖ <= 0.05` on `R11`, not closed here).
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R11 outer tier on the true radius. -/
theorem R11_budget_lt_app : (0.002 : ℝ) + 0.05 * R11.radius < 0.1 := by
  have h := R11_radius_lt
  have hM : 0.05 * R11.radius < 0.05 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R11_center_of_tenth_lower (hC : (0.1 : ℝ) ≤ ‖xiShifted R11.center‖) :
    (0.002 : ℝ) + 0.05 * R11.radius ≤ ‖xiShifted R11.center‖ := by
  have hB := R11_budget_lt_app
  linarith

def R11_center_residual_tenth : Prop :=
  (0.1 : ℝ) ≤ ‖xiShifted R11.center‖

theorem R11_center_obligation_of_residual_tenth (h : R11_center_residual_tenth) :
    (0.002 : ℝ) + 0.05 * R11.radius ≤ ‖xiShifted R11.center‖ :=
  R11_center_of_tenth_lower h

def R11_deriv_residual : Prop :=
  ∀ w, R11.mem w → ‖deriv xiShifted w‖ ≤ (0.05 : ℝ)

theorem R11_leaf_of_residuals (hc : R11_center_residual_tenth)
    (hd : R11_deriv_residual) : R11_leaf_obligations :=
  ⟨R11_center_obligation_of_residual_tenth hc, hd⟩

theorem R11_H_of_residuals (hc : R11_center_residual_tenth)
    (hd : R11_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (-10, -7.5, 0.1, 0.3)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
...
/-! ## ASSEMBLY-R36 leaf+H residual set + factor wall verdict (append-only, no redefinition)

Grep record (read-only, verified before writing):
- `R35` base is the read model (def 4513; coords 4516-4519; strip 4524-4525;
  radius_lt 4540 via sample_cell_radius_bound; mem 4543; leaf 4553-4555
  inner tier (0.15,0.06); H 4582-4592 with hc_mem + hc_eq).
- `R36` base present (def 4597-4598; coords 4600-4603; width 4605-4606;
  strip 4608-4609; dx 4611-4613; dy 4615-4617; radius_eq 4619-4622;
  radius_lt 4624-4625; mem 4627-4633 (0,2.5,0.3,0.49); leaf 4637-4639
  inner tier (0.15,0.06); fencing 4641-4643; H 4666-4676 with hc_mem + hc_eq).
- `fine_eps_inner_pos` 1034; `gridFine` 1002; `sample_cell_radius_bound` 421.
- BD small-r bridge for R36 is separate (BD_R36SmallR 14917-15207:
  C 5600 at 15057, M 700000 at 15061, threshold 882000.15 at 15173/15183).
  This block does not touch BD/BG namespaces.
- Grep-clean before writing: no `R36_budget_lt_app`,
  `R36_center_of_three_tenths_lower`, `R36_center_residual_three_tenths`,
  `R36_center_obligation_of_residual_three_tenths`, `R36_deriv_residual`,
  `R36_leaf_of_residuals`, `R36_H_of_residuals`, `R36_threshold_eq`,
  `R36_C_eq_app`, `R36_M_eq_app`, `R36_committed_product_lt`,
  `R36_required_Azeta_of_committed_56`, `R36_wall_infeasible_realistic`
  in file; R35 base left untouched (no duplication).

Leaf mirror for R36 inner tier (eps, M) = (0.15, 0.06) at
c36 = (0, 2.5, 0.3, 0.49): budget 0.15 + 0.06 * radius < 0.3 lifts
R36_radius_lt (same mul_lt_mul_of_pos_left + linarith shape as R06),
so 0.3 <= norm at R36.center discharges the center conjunct; deriv
residual is exactly the second conjunct; leaf_of rebuilds
R36_leaf_obligations; H_of discharges the gridFine H shape via
R36_H_instance (with hc_mem + hc_eq per 4666, matching R06 form).
Value-or-gap: budget 0.15 + 0.06 * 1.26 = 0.2256 < 0.3; full R36 H leaf
now conditional only on two named numeric enclosures (0.3 <= norm at
center + uniform norm deriv <= 0.06 on R36, not closed here).
...
closing the BD threshold 0.15 + 700000 * 1.26 = 882000.15 needs
Azeta >= 157500026785 (proved as R36_required_Azeta_of_committed_56),
...
far below 882000.15 (R36_committed_product_lt). Hence no O(1) Azeta
closes R36 at M = 700000; the two leaf enclosures above stay as the
residual. Zero cells claimed closed.
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R36 inner tier on the true radius. -/
theorem R36_budget_lt_app : (0.15 : ℝ) + 0.06 * R36.radius < 0.3 := by
  have h := R36_radius_lt
  have hM : 0.06 * R36.radius < 0.06 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R36_center_of_three_tenths_lower (hC : (0.3 : ℝ) ≤ ‖xiShifted R36.center‖) :
    (0.15 : ℝ) + 0.06 * R36.radius ≤ ‖xiShifted R36.center‖ := by
  have hB := R36_budget_lt_app
  linarith

def R36_center_residual_three_tenths : Prop :=
  (0.3 : ℝ) ≤ ‖xiShifted R36.center‖

theorem R36_center_obligation_of_residual_three_tenths
    (h : R36_center_residual_three_tenths) :
    (0.15 : ℝ) + 0.06 * R36.radius ≤ ‖xiShifted R36.center‖ :=
  R36_center_of_three_tenths_lower h

def R36_deriv_residual : Prop :=
  ∀ w, R36.mem w → ‖deriv xiShifted w‖ ≤ (0.06 : ℝ)

theorem R36_leaf_of_residuals (hc : R36_center_residual_three_tenths)
    (hd : R36_deriv_residual) : R36_leaf_obligations :=
  ⟨R36_center_obligation_of_residual_three_tenths hc, hd⟩

theorem R36_H_of_residuals (hc : R36_center_residual_three_tenths)
    (hd : R36_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (0, 2.5, 0.3, 0.49)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
      ...
      0 < ε ∧ (∀ w, R.mem w → ‖deriv xiShifted w‖ ≤ M) ∧
      ε + M * R.radius ≤ ‖xiShifted R.center‖ :=
  R36_H_instance (R36_leaf_of_residuals hc hd) c hc_mem hc_eq

/-- Small-r threshold value for R36 (inner tier, M = 700000). -/
theorem R36_threshold_eq : (0.15 : ℝ) + 700000 * 1.26 = 882000.15 := by
  norm_num

/-- Disc factor product for R36 (poly 56 tier). -/
theorem R36_C_eq_app : (56 : ℝ) * 1 * 10 * 10 = 5600 := by
  norm_num

/-- Cauchy M for R36 (same small-r numbers as BD bridge). -/
theorem R36_M_eq_app : (5600 : ℝ) / 0.008 = 700000 := by
  norm_num

/-- Committed product far below threshold (exact numbers). -/
theorem R36_committed_product_lt :
    56 * 1 * (1 / 10000000) * (10 : ℝ) < (0.15 : ℝ) + 700000 * 1.26 := by
  norm_num

/-- With committed Agam, closing needs Azeta >= 157500026785 (exact floor). -/
theorem R36_required_Azeta_of_committed_56 {Azeta : ℝ}
...
/-- Wall verdict: no realistic O(1) Azeta closes R36 at M = 700000. -/
theorem R36_wall_infeasible_realistic {Azeta : ℝ} (hle : Azeta ≤ 10)
    (h : (0.15 : ℝ) + 700000 * 1.26 ≤ 56 * 1 * (1 / 10000000) * Azeta) :
    False := by
  have hreq := R36_required_Azeta_of_committed_56 h
  linarith

end CentralCoverAssembly

/-! ## ASSEMBLY-R37 leaf+H residual set + factor wall verdict (append-only, no redefinition)
...
- `R37` base present (mid tier `(0.05,0.07)`): def 4681-4682;
  coords 4684-4687 (`R37_x0/x1/y0/y1`); strip 4692-4693 (`R37_strip_lo/hi`);
  dx 4695-4697; dy 4699-4701; radius_eq 4703-4706; radius_lt 4708-4709
  (`R37.radius < 1.26` via `sample_cell_radius_bound` 421);
  mem 4711-4717 (`(2,4.5,0.3,0.49)` in `gridFine` 1002);
  leaf 4721-4723 (`R37_leaf_obligations`: center `0.05 + 0.07 * radius`
  AND deriv `forall w, mem -> norm deriv <= 0.07`); fencing 4725-4727;
  H 4750-4760 (`R37_H_instance` with `hc_mem + hc_eq`, `fine_eps_mid_pos` 1033).
- Small-r numbers for R37 filed separately (BG 12961/12965, BD 15389/15393:
  C 5600, M 700000 at `0.008`). This block does not touch BG/BD namespaces.
- Grep-clean before writing: no `R37_budget_lt_app`,
  `R37_center_of_two_tenths_lower`, `R37_center_residual_two_tenths`,
  `R37_center_obligation_of_residual_two_tenths`, `R37_deriv_residual`,
  `R37_leaf_of_residuals`, `R37_H_of_residuals`, `R37_threshold_eq`,
  `R37_C_eq_app`, `R37_M_eq_app`, `R37_committed_product_lt`,
  `R37_required_Azeta_of_committed_56`, `R37_wall_infeasible_realistic`
  in file; R36 base left untouched (no duplication).

Leaf mirror for R37 mid tier (eps, M) = (0.05, 0.07) at
c37 = (2, 4.5, 0.3, 0.49): budget 0.05 + 0.07 * radius < 0.2 lifts
R37_radius_lt (same mul_lt_mul_of_pos_left + linarith shape as R36),
so 0.2 <= norm at R37.center discharges the center conjunct; deriv
residual is exactly the second conjunct; leaf_of rebuilds
R37_leaf_obligations; H_of discharges the gridFine H shape via
R37_H_instance (with hc_mem + hc_eq per 4750, matching R36 form).
Value-or-gap: budget 0.05 + 0.07 * 1.26 = 0.1382 < 0.2; full R37 H leaf
now conditional only on two named numeric enclosures (0.2 <= norm at
center + uniform norm deriv <= 0.07 on R37, not closed here).

Wall verdict (infeasible at filed small-r numbers, exact numerals below):
with disc factors Apoly = 56, Api = 1 and committed Agam = 1/10000000,
closing the mid-tier threshold 0.05 + 700000 * 1.26 = 882000.05 needs
Azeta >= 157500008928 (proved as R37_required_Azeta_of_committed_56),
while the realistic O(1) range has Azeta <= 10 (true zeta is O(1) on
this rect); the committed product 56*1*(1/10000000)*10 = 0.000056 is
far below 882000.05 (R37_committed_product_lt). Hence no O(1) Azeta
closes R37 at M = 700000; the two leaf enclosures above stay as the
residual. Zero cells claimed closed.
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R37 mid tier on the true radius. -/
theorem R37_budget_lt_app : (0.05 : ℝ) + 0.07 * R37.radius < 0.2 := by
  have h := R37_radius_lt
  have hM : 0.07 * R37.radius < 0.07 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R37_center_of_two_tenths_lower (hC : (0.2 : ℝ) ≤ ‖xiShifted R37.center‖) :
    (0.05 : ℝ) + 0.07 * R37.radius ≤ ‖xiShifted R37.center‖ := by
  have hB := R37_budget_lt_app
  linarith

def R37_center_residual_two_tenths : Prop :=
  (0.2 : ℝ) ≤ ‖xiShifted R37.center‖

theorem R37_center_obligation_of_residual_two_tenths
    (h : R37_center_residual_two_tenths) :
    (0.05 : ℝ) + 0.07 * R37.radius ≤ ‖xiShifted R37.center‖ :=
  R37_center_of_two_tenths_lower h

def R37_deriv_residual : Prop :=
  ∀ w, R37.mem w → ‖deriv xiShifted w‖ ≤ (0.07 : ℝ)

theorem R37_leaf_of_residuals (hc : R37_center_residual_two_tenths)
    (hd : R37_deriv_residual) : R37_leaf_obligations :=
  ⟨R37_center_obligation_of_residual_two_tenths hc, hd⟩

theorem R37_H_of_residuals (hc : R37_center_residual_two_tenths)
    (hd : R37_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (2, 4.5, 0.3, 0.49)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
      ...
      0 < ε ∧ (∀ w, R.mem w → ‖deriv xiShifted w‖ ≤ M) ∧
      ε + M * R.radius ≤ ‖xiShifted R.center‖ :=
  R37_H_instance (R37_leaf_of_residuals hc hd) c hc_mem hc_eq

/-- Small-r threshold value for R37 (mid tier, M = 700000). -/
theorem R37_threshold_eq : (0.05 : ℝ) + 700000 * 1.26 = 882000.05 := by
  norm_num

/-- Disc factor product for R37 (poly 56 tier). -/
theorem R37_C_eq_app : (56 : ℝ) * 1 * 10 * 10 = 5600 := by
  norm_num

/-- Cauchy M for R37 (same small-r numbers as filed bridge). -/
theorem R37_M_eq_app : (5600 : ℝ) / 0.008 = 700000 := by
  norm_num

/-- Committed product far below threshold (exact numbers). -/
theorem R37_committed_product_lt :
    56 * 1 * (1 / 10000000) * (10 : ℝ) < (0.05 : ℝ) + 700000 * 1.26 := by
  norm_num

/-- With committed Agam, closing needs Azeta >= 157500008928 (exact floor). -/
theorem R37_required_Azeta_of_committed_56 {Azeta : ℝ}
...
/-- Wall verdict: no realistic O(1) Azeta closes R37 at M = 700000. -/
theorem R37_wall_infeasible_realistic {Azeta : ℝ} (hle : Azeta ≤ 10)
    (h : (0.05 : ℝ) + 700000 * 1.26 ≤ 56 * 1 * (1 / 10000000) * Azeta) :
    False := by
  have hreq := R37_required_Azeta_of_committed_56 h
  linarith

end CentralCoverAssembly

/-! ## ASSEMBLY-R38 leaf+H residual set + factor wall verdict (append-only, no redefinition)

Grep record (read-only, verified before writing):
- `R37` residual+wall is the shape model (18310-18443): `R37_budget_lt_app` 18362,
  `R37_center_of_two_tenths_lower` 18368, `R37_center_residual_two_tenths` 18373,
  `R37_center_obligation_of_residual_two_tenths` 18376, `R37_deriv_residual` 18381,
  `R37_leaf_of_residuals` 18384, `R37_H_of_residuals` 18388, `R37_threshold_eq` 18400,
  `R37_C_eq_app` 18404, `R37_M_eq_app` 18408, `R37_committed_product_lt` 18412,
  `R37_required_Azeta_of_committed_56` 18417, `R37_wall_infeasible_realistic` 18437.
- `R38` base present (mid tier `(0.05,0.07)`): def 4765-4766;
  coords 4768-4771 (`R38_x0/x1/y0/y1`); strip 4776-4777 (`R38_strip_lo/hi`);
  dx 4779-4781; dy 4783-4785; radius_eq 4787-4790; radius_lt 4792-4793
  (`R38.radius < 1.26` via `sample_cell_radius_bound` 421);
  mem 4795-4801 (`(4,6.5,0.3,0.49)` in `gridFine` 1002);
  leaf 4805-4807 (`R38_leaf_obligations`: center `0.05 + 0.07 * radius`
  AND deriv `forall w, mem -> norm deriv <= 0.07`); fencing 4809-4811;
  H 4834-4844 (`R38_H_instance` with `hc_mem + hc_eq`, `fine_eps_mid_pos` 1033).
- Small-r numbers for R38 filed separately (BG 13236/13240, BD 15681/15685:
  C 5600, M 700000 at `0.008`). This block does not touch BG/BD namespaces.
- Grep-clean before writing: no `R38_budget_lt_app`,
  `R38_center_of_two_tenths_lower`, `R38_center_residual_two_tenths`,
  `R38_center_obligation_of_residual_two_tenths`, `R38_deriv_residual`,
  `R38_leaf_of_residuals`, `R38_H_of_residuals`, `R38_threshold_eq`,
  `R38_C_eq_app`, `R38_M_eq_app`, `R38_committed_product_lt`,
  `R38_required_Azeta_of_committed_56`, `R38_wall_infeasible_realistic`
  in file; R37 base left untouched (no duplication).

Leaf mirror for R38 mid tier (eps, M) = (0.05, 0.07) at
c38 = (4, 6.5, 0.3, 0.49): budget 0.05 + 0.07 * radius < 0.2 lifts
R38_radius_lt (same mul_lt_mul_of_pos_left + linarith shape as R37),
so 0.2 <= norm at R38.center discharges the center conjunct; deriv
residual is exactly the second conjunct; leaf_of rebuilds
R38_leaf_obligations; H_of discharges the gridFine H shape via
R38_H_instance (with hc_mem + hc_eq per 4834, matching R37 form).
Value-or-gap: budget 0.05 + 0.07 * 1.26 = 0.1382 < 0.2; full R38 H leaf
now conditional only on two named numeric enclosures (0.2 <= norm at
center + uniform norm deriv <= 0.07 on R38, not closed here).

Wall verdict (infeasible at filed small-r numbers, exact numerals below):
with disc factors Apoly = 56, Api = 1 and committed Agam = 1/10000000,
closing the mid-tier threshold 0.05 + 700000 * 1.26 = 882000.05 needs
Azeta >= 157500008928 (proved as R38_required_Azeta_of_committed_56),
while the realistic O(1) range has Azeta <= 10 (true zeta is O(1) on
this rect); the committed product 56*1*(1/10000000)*10 = 0.000056 is
far below 882000.05 (R38_committed_product_lt). Hence no O(1) Azeta
closes R38 at M = 700000; the two leaf enclosures above stay as the
residual. Zero cells claimed closed.
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R38 mid tier on the true radius. -/
theorem R38_budget_lt_app : (0.05 : ℝ) + 0.07 * R38.radius < 0.2 := by
  have h := R38_radius_lt
  have hM : 0.07 * R38.radius < 0.07 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R38_center_of_two_tenths_lower (hC : (0.2 : ℝ) ≤ ‖xiShifted R38.center‖) :
    (0.05 : ℝ) + 0.07 * R38.radius ≤ ‖xiShifted R38.center‖ := by
  have hB := R38_budget_lt_app
  linarith

def R38_center_residual_two_tenths : Prop :=
  (0.2 : ℝ) ≤ ‖xiShifted R38.center‖

theorem R38_center_obligation_of_residual_two_tenths
    (h : R38_center_residual_two_tenths) :
    (0.05 : ℝ) + 0.07 * R38.radius ≤ ‖xiShifted R38.center‖ :=
  R38_center_of_two_tenths_lower h

def R38_deriv_residual : Prop :=
  ∀ w, R38.mem w → ‖deriv xiShifted w‖ ≤ (0.07 : ℝ)

theorem R38_leaf_of_residuals (hc : R38_center_residual_two_tenths)
    (hd : R38_deriv_residual) : R38_leaf_obligations :=
  ⟨R38_center_obligation_of_residual_two_tenths hc, hd⟩

theorem R38_H_of_residuals (hc : R38_center_residual_two_tenths)
    (hd : R38_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (4, 6.5, 0.3, 0.49)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
      ...
      0 < ε ∧ (∀ w, R.mem w → ‖deriv xiShifted w‖ ≤ M) ∧
      ε + M * R.radius ≤ ‖xiShifted R.center‖ :=
  R38_H_instance (R38_leaf_of_residuals hc hd) c hc_mem hc_eq

/-- Small-r threshold value for R38 (mid tier, M = 700000). -/
theorem R38_threshold_eq : (0.05 : ℝ) + 700000 * 1.26 = 882000.05 := by
  norm_num

/-- Disc factor product for R38 (poly 56 tier). -/
theorem R38_C_eq_app : (56 : ℝ) * 1 * 10 * 10 = 5600 := by
  norm_num

/-- Cauchy M for R38 (same small-r numbers as filed bridge). -/
theorem R38_M_eq_app : (5600 : ℝ) / 0.008 = 700000 := by
  norm_num

/-- Committed product far below threshold (exact numbers). -/
theorem R38_committed_product_lt :
    56 * 1 * (1 / 10000000) * (10 : ℝ) < (0.05 : ℝ) + 700000 * 1.26 := by
  norm_num

/-- With committed Agam, closing needs Azeta >= 157500008928 (exact floor). -/
theorem R38_required_Azeta_of_committed_56 {Azeta : ℝ}
    (h : (0.05 : ℝ) + 700000 * 1.26 ≤ 56 * 1 * (1 / 10000000) * Azeta) :
    (157500008928 : ℝ) ≤ Azeta := by
  have hb : (0.05 : ℝ) + 700000 * 1.26 = 882000.05 := by norm_num
  rw [hb] at h
...
/-- Wall verdict: no realistic O(1) Azeta closes R38 at M = 700000. -/
theorem R38_wall_infeasible_realistic {Azeta : ℝ} (hle : Azeta ≤ 10)
    (h : (0.05 : ℝ) + 700000 * 1.26 ≤ 56 * 1 * (1 / 10000000) * Azeta) :
    False := by
  have hreq := R38_required_Azeta_of_committed_56 h
  linarith

end CentralCoverAssembly

/-! ## ASSEMBLY-R39 leaf+H residual set + factor wall verdict (append-only, no redefinition)

Grep record (read-only, verified before writing):
- `R38` residual+wall is the shape model (18494-18578): `R38_budget_lt_app` 18497,
  `R38_center_of_two_tenths_lower` 18503, `R38_center_residual_two_tenths` 18508,
  `R38_center_obligation_of_residual_two_tenths` 18511, `R38_deriv_residual` 18516,
  `R38_leaf_of_residuals` 18519, `R38_H_of_residuals` 18523, `R38_threshold_eq` 18535,
  `R38_C_eq_app` 18539, `R38_M_eq_app` 18543, `R38_committed_product_lt` 18547,
  `R38_required_Azeta_of_committed_56` 18552, `R38_wall_infeasible_realistic` 18572.
- `R39` base present (outer tier `(0.002,0.07)` leaf): def 4849-4850;
  coords 4852-4855 (`R39_x0/x1/y0/y1`); strip 4860-4861 (`R39_strip_lo/hi`);
  dx 4863-4865; dy 4867-4869; radius_eq 4871-4874; radius_lt 4876-4877
  (`R39.radius < 1.26` via `sample_cell_radius_bound` 421);
  mem 4879-4885 (`(6,8.5,0.3,0.49)` in `gridFine` 1002);
  leaf 4889-4891 (`R39_leaf_obligations`: center `0.002 + 0.07 * radius`
  AND deriv `forall w, mem -> norm deriv <= 0.07`); fencing 4893-4895;
  H 4918-4928 (`R39_H_instance` with `hc_mem + hc_eq`, `fine_eps_outer_pos`).
- Small-r numbers for R39 filed separately (BG 13534/13538, BD 16013/16017:
  C 5600, M 700000 at `0.008`). This block does not touch BG/BD namespaces.
- Grep-clean before writing: no `R39_budget_lt_app`,
  `R39_center_of_two_tenths_lower`, `R39_center_residual_two_tenths`,
  `R39_center_obligation_of_residual_two_tenths`, `R39_deriv_residual`,
  `R39_leaf_of_residuals`, `R39_H_of_residuals`, `R39_threshold_eq`,
  `R39_C_eq_app`, `R39_M_eq_app`, `R39_committed_product_lt`,
  `R39_required_Azeta_of_committed_56`, `R39_wall_infeasible_realistic`
  in file; R38 base left untouched (no duplication).

Leaf mirror for R39 outer tier (eps, M) = (0.002, 0.07) at
c39 = (6, 8.5, 0.3, 0.49): budget 0.002 + 0.07 * radius < 0.2 lifts
R39_radius_lt (same mul_lt_mul_of_pos_left + linarith shape as R38),
so 0.2 <= norm at R39.center discharges the center conjunct; deriv
residual is exactly the second conjunct; leaf_of rebuilds
R39_leaf_obligations; H_of discharges the gridFine H shape via
R39_H_instance (with hc_mem + hc_eq per 4918, matching R38 form).
Value-or-gap: budget 0.002 + 0.07 * 1.26 = 0.0902 < 0.2; full R39 H leaf
now conditional only on two named numeric enclosures (0.2 <= norm at
center + uniform norm deriv <= 0.07 on R39, not closed here).

Wall verdict (infeasible at filed small-r numbers, exact numerals below):
with disc factors Apoly = 56, Api = 1 and committed Agam = 1/10000000,
closing the outer-tier threshold 0.002 + 700000 * 1.26 = 882000.002 needs
Azeta >= 157500000357 (proved as R39_required_Azeta_of_committed_56),
while the realistic O(1) range has Azeta <= 10 (true zeta is O(1) on
this rect); the committed product 56*1*(1/10000000)*10 = 0.000056 is
far below 882000.002 (R39_committed_product_lt). Hence no O(1) Azeta
closes R39 at M = 700000; the two leaf enclosures above stay as the
residual. Zero cells claimed closed.
-/

namespace CentralCoverAssembly

/-- Numeric budget for the R39 outer tier on the true radius. -/
theorem R39_budget_lt_app : (0.002 : ℝ) + 0.07 * R39.radius < 0.2 := by
  have h := R39_radius_lt
  have hM : 0.07 * R39.radius < 0.07 * 1.26 :=
    mul_lt_mul_of_pos_left h (by norm_num)
  linarith

theorem R39_center_of_two_tenths_lower (hC : (0.2 : ℝ) ≤ ‖xiShifted R39.center‖) :
    (0.002 : ℝ) + 0.07 * R39.radius ≤ ‖xiShifted R39.center‖ := by
  have hB := R39_budget_lt_app
  linarith

def R39_center_residual_two_tenths : Prop :=
  (0.2 : ℝ) ≤ ‖xiShifted R39.center‖

theorem R39_center_obligation_of_residual_two_tenths
    (h : R39_center_residual_two_tenths) :
    (0.002 : ℝ) + 0.07 * R39.radius ≤ ‖xiShifted R39.center‖ :=
  R39_center_of_two_tenths_lower h

def R39_deriv_residual : Prop :=
  ∀ w, R39.mem w → ‖deriv xiShifted w‖ ≤ (0.07 : ℝ)

theorem R39_leaf_of_residuals (hc : R39_center_residual_two_tenths)
    (hd : R39_deriv_residual) : R39_leaf_obligations :=
  ⟨R39_center_obligation_of_residual_two_tenths hc, hd⟩

theorem R39_H_of_residuals (hc : R39_center_residual_two_tenths)
    (hd : R39_deriv_residual)
    (c : ℝ × ℝ × ℝ × ℝ) (hc_mem : c ∈ gridFine)
    (hc_eq : c = (6, 8.5, 0.3, 0.49)) :
    ∃ (R : CellProofEngine.Rect2D) (ε M : ℝ),
      R.x0 = c.1 ∧ R.x1 = c.2.1 ∧ R.y0 = c.2.2.1 ∧ R.y1 = c.2.2.2 ∧
      ...
      0 < ε ∧ (∀ w, R.mem w → ‖deriv xiShifted w‖ ≤ M) ∧
      ε + M * R.radius ≤ ‖xiShifted R.center‖ :=
  R39_H_instance (R39_leaf_of_residuals hc hd) c hc_mem hc_eq

/-- Small-r threshold value for R39 (outer tier, M = 700000). -/
theorem R39_threshold_eq : (0.002 : ℝ) + 700000 * 1.26 = 882000.002 := by
  norm_num

/-- Disc factor product for R39 (poly 56 tier). -/
theorem R39_C_eq_app : (56 : ℝ) * 1 * 10 * 10 = 5600 := by
  norm_num

/-- Cauchy M for R39 (same small-r numbers as filed bridge). -/
theorem R39_M_eq_app : (5600 : ℝ) / 0.008 = 700000 := by
  norm_num

/-- Committed product far below threshold (exact numbers). -/
theorem R39_committed_product_lt :
    56 * 1 * (1 / 10000000) * (10 : ℝ) < (0.002 : ℝ) + 700000 * 1.26 := by
  norm_num

/-- With committed Agam, closing needs Azeta >= 157500000357 (exact floor). -/
theorem R39_required_Azeta_of_committed_56 {Azeta : ℝ}
...
/-- Wall verdict: no realistic O(1) Azeta closes R39 at M = 700000. -/
theorem R39_wall_infeasible_realistic {Azeta : ℝ} (hle : Azeta ≤ 10)
    (h : (0.002 : ℝ) + 700000 * 1.26 ≤ 56 * 1 * (1 / 10000000) * Azeta) :
    False := by
  have hreq := R39_required_Azeta_of_committed_56 h
  linarith
